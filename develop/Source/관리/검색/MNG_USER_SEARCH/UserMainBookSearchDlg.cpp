...
//*/ END -----------------------------------------------------------------------
		...
		) 
		{
			...
			//if ( sData != _T("대출중") ) {
			if(sData.Compare(_T("대출중")) != 0 && sData.Compare(_T("책두레진행")) != 0){
...
INT CUserMainBookSearchDlg::GetSqlSearchALLBookInfo(CString &sWhere, INT nMode)
{
	...
	// 2009.12.16 ADD BY PWR : 책만 검색되도록 (부록검색안함)
	if(0 == nMode)
	{
		// 17/07/20 김혜영 : 책나래대출자료 상태 추가 
/*//BEFORE1 -----------------------------------------------------------------------------
		// 2013.12.01 UPDATE BY BJS : 2013도서관통합서비스확대구축및개선 (통합도서타관대출 상태 추가)
		sWhere += _T(" BB.WORKING_STATUS IN ('BOL112N','BOL113O','BOL114O','BOL115O','BOL211O','BOL212O','BOL213O','BOL214O','BOL215O','BOL217O','BOL218O','BOL511O','BOL511O')");
		//sWhere += _T(" BB.WORKING_STATUS IN ('BOL112N','BOL113O','BOL114O','BOL115O','BOL211O','BOL212O','BOL213O','BOL214O','BOL215O','BOL217O','BOL218O','BOL511O')");
////BEFORE2 -----------------------------------------------------------------------------
		//16/03/09 김혜영 : 통합도서타관대출 상태를 추가했으나 BOL511O으로 추가하여 BOL611O으로 수정
		sWhere += _T(" BB.WORKING_STATUS IN ('BOL112N','BOL113O','BOL114O','BOL115O','BOL211O','BOL212O','BOL213O','BOL214O','BOL215O','BOL217O','BOL218O','BOL511O','BOL611O')");
*///AFTER----------------------------------------------------------------------------------	
		// 18.09.27 JMJ KOL.RED.2018.008
...
/*//END-----------------------------------------------------------------------------------*/	
		sWhere += _T(" AND BB.BOOK_APPENDIX_FLAG='B'");
	...
	else
	{
		...
		sWhere += _T(" AND BB.BOOK_TYPE IN ('B','C') ");
	}
