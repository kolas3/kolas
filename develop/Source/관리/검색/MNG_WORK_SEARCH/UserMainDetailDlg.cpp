...
void CUserMainDetailDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_DETAIL_stcSeriesStatement, m_MarkStaticSeries);
...
HBRUSH CUserMainDetailDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	//2012.11.05 UPDATE BY BJY : 2012년도 도서관 통합서비스 확대구축 사업 2차
	if( IDC_DETAIL_edtTitle == pWnd->GetDlgCtrlID() ||
		...
		IDC_DETAIL_edtSeriesStatement == pWnd->GetDlgCtrlID() )
	{
...
INT CUserMainDetailDlg::ShowDetailView()
{
	...
	// 2009.12.21 ADD BY PWR : 업무용 상세보기 똑같이 검색되도록 수정
	if(_T("MO") == m_sPublishFormCode)
	{
		where += _T(" AND B.WORKING_STATUS NOT IN ('BOT211O','BOT212O') ");
	}
...
INT CUserMainDetailDlg::DisplaySpecies()
{
	...
	//2012.11.05 ADD BY BJY : 2012년도 도서관 통합서비스 확대구축 사업 2차
	CString strSeriesStatement = _T("");
	m_MarcConvertMgr.GetSeriesStatement(&Marc, strSeriesStatement);
	GetDlgItem(IDC_DETAIL_edtSeriesStatement)->SetWindowText(strSeriesStatement);
