	...
	: CESL_Mgr(IDD, pParent)
{
	...
	m_CMName = _T("CM_CCLASS_SIGN_MANAGER_01");
	m_DMName_IDX = _T("DM_CCLASS_SIGN_MANAGER_IDX");
	m_DMName_DDC = _T("DM_CCLASS_SIGN_MANAGER_DDC");
	m_DMName_KDC = _T("DM_CCLASS_SIGN_MANAGER_KDC");
}

CCLASS_SIGN_MANAGER_03::~CCLASS_SIGN_MANAGER_03()
{
	// 17/03/09 김혜영 : 공통>분류기호및주제명검색요어>분류기호에서 본문주기 팝업에서 메모리 누수 발생
//*/ ADD ---------------------------------------------------------------------------
	if( m_checkedRow )
		free( m_checkedRow );
	m_checkedRow = NULL;
...
VOID CCLASS_SIGN_MANAGER_03::OnShowWindow(BOOL bShow, UINT nStatus) 
{
	...
	// 17/03/09 김혜영 : 공통>분류기호및주제명검색요어>분류기호에서 본문주기 팝업이 생성되지 않음
	// 괄호처리가 잘못되어 무조건 팝업을 닫는 현상이 발생
/*//BEFORE-----------------------------------------------------------------------------
	if( -1 == ids ){}
...
HBRUSH CCLASS_SIGN_MANAGER_03::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
