...
BOOL CMNG_Common_EnvironmentMgr::Init()
{
	...
	// 통합시스템 적용
	strQuery.Format(_T("GROUP_2='기타' AND SYS_SETUP_YN='N' AND MANAGE_CODE='%s'"), m_pInfo->MANAGE_CODE );
	...
	{
		if(m_pDM->GetCellData(_T("관리항목"), i) == _T("대출자공유관리") ||
			// 2013.11.13 UPDATE BY PWR : 2013도서관통합서비스확대구축및개선 (대출반납제어추가)
			m_pDM->GetCellData(_T("관리항목"), i) == _T("공동목록공지사항") ||
			m_pDM->GetCellData(_T("관리항목"), i) == _T("대출반납제어"))
//				m_pDM->GetCellData(_T("관리항목"), i) == _T("공동목록공지사항"))
		{
			strTmpData = m_pDM->GetCellData(_T("관리값"), i);
			m_pGrid->SetTextMatrix(i+1,3,strTmpData);
			strTmpData = m_pDM->GetCellData(_T("관리값_1"), i);
			if(strTmpData==_T("Y")) strTmpData=_T("사용함");
			else if(strTmpData==_T("N")) strTmpData=_T("사용안함");
			m_pGrid->SetTextMatrix(i+1,5,strTmpData);
		}
		else if(m_pDM->GetCellData(_T("관리항목"), i) == _T("센터업로드사용여부")||
			m_pDM->GetCellData(_T("관리항목"), i) == _T("메일일괄발송시간설정"))
		{
			strTmpData = m_pDM->GetCellData(_T("관리값_1"), i);
			if(strTmpData==_T("Y")) strTmpData=_T("사용함");
			else if(strTmpData==_T("N")) strTmpData=_T("사용안함");
		...
		// 2009.05.22 ADD BY PWR : 정렬기준 추가 (대출반납 화면의 현황그리드)
		else if(m_pDM->GetCellData(_T("관리항목"), i) == _T("정렬기준"))
		{
			strTmpData = m_pDM->GetCellData(_T("관리값_1"), i);
...
VOID CMNG_Common_EnvironmentMgr::SaveManageData()
{
	...
	for(INT i=0 ; i<cnt ; i++)
	{
		...
		if(strTmp==_T("메일일괄발송시간설정"))
		{
			strTmp=m_pGrid->GetTextMatrix(i,5);
			m_pDM->SetCellData(_T("관리값_1"), m_pGrid->GetTextMatrix(i,5), i-1);
		}
		else if(strTmp==_T("상호대차관리"))
		{
			strTmp=m_pGrid->GetTextMatrix(i,5);
			m_pDM->SetCellData(_T("관리값"),  m_pGrid->GetTextMatrix(i,5), i-1);
		}
	}
...
BOOL CMNG_Common_EnvironmentMgr::PreTranslateMessage(MSG* pMsg) 
{
	...
	if (ESL_GRID_MODIFY == pMsg->message)
	{
		...
		{	
			...
			if(m_pDM->GetCellData(_T("관리항목"), nRow) != _T("대출자공유관리") &&
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("공동목록공지사항") &&
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("센터업로드사용여부") &&
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("메일일괄발송시간설정") &&
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("상호대차관리") && 
				// 2013.11.13 UPDATE BY PWR : 2013도서관통합서비스확대구축및개선 (대출반납제어추가)
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("정렬기준") && 
				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("대출반납제어"))
//				m_pDM->GetCellData(_T("관리항목"), nRow) != _T("정렬기준")) // 2009.05.22 ADD BY PWR : 정렬기준 추가 (대출반납 화면의 현황그리드)
			{
...
VOID CMNG_Common_EnvironmentMgr::OnDblClickgridCOMMONENVIRONMENTMGR() 
{
	...
	if(m_pDM->GetCellData(_T("관리항목"), nRow) == _T("자료찾기검색형식"))
	{
		...
		return ;
		    ...
		    m_pDM->GetCellData(_T("관리항목"), nRow) == _T("메일일괄발송")			   ||
			m_pDM->GetCellData(_T("관리항목"), nRow) == _T("자산통계 제적자료포함")	   )
	{
		strTmpData = m_pDM->GetCellData(_T("관리값"), nRow);
	...
	else if(m_pDM->GetCellData(_T("관리항목"), nRow) == _T("대출자공유관리") ||
				m_pDM->GetCellData(_T("관리항목"), nRow) == _T("공동목록공지사항") ||
				// 2013.11.13 UPDATE BY PWR : 2013도서관통합서비스확대구축및개선 (대출반납제어추가)
				m_pDM->GetCellData(_T("관리항목"), nRow) == _T("센터업로드사용여부") ||
				m_pDM->GetCellData(_T("관리항목"), nRow) == _T("대출반납제어"))
//				m_pDM->GetCellData(_T("관리항목"), nRow) == _T("센터업로드사용여부"))
	{
		strTmpData = m_pDM->GetCellData(_T("관리값_1"), nRow);
		if(strTmpData == _T("Y")) strTmpData = _T("N");
		else if(strTmpData == _T("N")) strTmpData = _T("Y");
		m_pDM->SetCellData(_T("관리값_1"), strTmpData, nRow);
...
//					m_pDM->GetCellData(_T("관리항목"), i) == _T("공동목록공지사항"))
	...
	// 2009.05.22 ADD BY PWR : 정렬기준 추가 (대출반납 화면의 현황그리드)
	else if(m_pDM->GetCellData(_T("관리항목"), nRow) == _T("정렬기준"))
	{
		strTmpData = m_pDM->GetCellData(_T("관리값_1"), nRow);
