// BO_CAT_WORD.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE BO_CAT_WORDDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("BO_CAT_WORD.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(BO_CAT_WORDDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(BO_CAT_WORDDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("BO_CAT_WORD.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(BO_CAT_WORDDLL);
	}
	return 1;   // ok
}

CBO_CAT_WORD_0000* m_pWindow=NULL;

__declspec(dllexport) VOID CreateForm(CWnd *pParent, CESL_Information* pInfo) 
{
	m_pWindow = NULL;
	m_pWindow = new CBO_CAT_WORD_0000(NULL);
...
__declspec(dllexport) CESL_Mgr* CreateModelessDlg(CString strSM, CString strButtonAlias, BOOL bReadOnly)
{
	...
	if (strSM == _T("검색"))
	{
		CBO_CAT_WORD_0100 *pDlg = NULL;
		pDlg = new CBO_CAT_WORD_0100(m_pWindow);
	...
	{
		CBO_CAT_WORD_0200 *pDlg = NULL;
		pDlg = new CBO_CAT_WORD_0200(m_pWindow);
		pDlg->pMain = m_pWindow->pMain;
		pDlg->pParentMgr = (CESL_Mgr*)m_pWindow;
		if (bReadOnly) pDlg->SM_STYLE = 'R';
		pDlg->m_pInfo = m_pWindow->m_pInfo;
	...
	{
		CBO_CAT_WORD_0300 *pDlg = NULL;
		pDlg = new CBO_CAT_WORD_0300(m_pWindow);
		pDlg->pMain = m_pWindow->pMain;
		pDlg->pParentMgr = (CESL_Mgr*)m_pWindow;
		if (bReadOnly) pDlg->SM_STYLE = 'R';
		pDlg->m_pInfo = m_pWindow->m_pInfo;
