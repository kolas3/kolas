...
BEGIN_MESSAGE_MAP(CBOVolumeDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_bVOLUME_SAVE, OnbSAVE)
	...
	ON_BN_CLICKED(IDC_bVOLUME_CLOSE, OnbCLOSE)
	...
	ON_EN_SETFOCUS(IDC_ePRICE_OTHRE_INFO, OnSetfocusePRICEOTHREINFO)
	ON_EN_SETFOCUS(IDC_ePHYSICAL_PROPERTIES, OnSetfocusePHYSICALPROPERTIES)
	ON_EN_SETFOCUS(IDC_eAUTHOR, OnSetfocuseAUTHOR)
	ON_EN_SETFOCUS(IDC_eAPPENDIX, OnSetfocuseAPPENDIX)
	ON_EN_SETFOCUS(IDC_ePAGE_CNT, OnSetfocusePAGECNT)
	ON_EN_SETFOCUS(IDC_eREG_CLASS_CODE, OnSetfocuseREGCLASSCODE)
	ON_EN_SETFOCUS(IDC_eSIZE, OnSetfocuseSIZE)
	ON_EN_SETFOCUS(IDC_eVOL, OnSetfocuseVOL)
	ON_EN_SETFOCUS(IDC_eVOLTITLE, OnSetfocuseVOLTITLE)
	ON_WM_CTLCOLOR()
	ON_EN_KILLFOCUS(IDC_edtVOL_SORT_NO, OnKillfocusedtVOLSORTNO)
	ON_EN_KILLFOCUS(IDC_eVOL, OnKillfocuseVOL)
	ON_EN_KILLFOCUS(IDC_eVOLTITLE, OnKillfocuseVOLTITLE)
	ON_EN_KILLFOCUS(IDC_eAUTHOR, OnKillfocuseAUTHOR)
	ON_EN_KILLFOCUS(IDC_ePUBLISH_YEAR, OnKillfocusePUBLISHYEAR)
	ON_EN_KILLFOCUS(IDC_eEA_ISBN, OnKillfocuseEAISBN)
	ON_EN_KILLFOCUS(IDC_eISBN2, OnKillfocuseISBN2)
	ON_EN_KILLFOCUS(IDC_eISBN3, OnKillfocuseISBN3)
	ON_EN_KILLFOCUS(IDC_ePHYSICAL_PROPERTIES, OnKillfocusePHYSICALPROPERTIES)
	ON_EN_KILLFOCUS(IDC_eAPPENDIX, OnKillfocuseAPPENDIX)
	ON_EN_KILLFOCUS(IDC_ePRICE_OTHRE_INFO, OnKillfocusePRICEOTHREINFO)
	//}}AFX_MSG_MAP
...
INT CBOVolumeDlg::Display()
{
	...
	CString strDmData = _T("");
	if( 0 >= m_pDM_Volume->GetRowCount() )
	{
		m_pCM->GetControlMgrData(_T("BB_낱권ISBN"), strDmData);
		if( strDmData.IsEmpty() )
		{
			m_pCM->SetControlMgrData(_T("BB_낱권ISBN"), _T("97889"));
		}
	}
	else if( 1 == m_pDM_Volume->GetRowCount() )
	{
		CString strData = _T("");
		m_pDM_Book->GetCellData(_T("BB_DB_FLAG"), 0, strData);
		if( _T("I") == strData )
		{
			m_pCM->GetControlMgrData(_T("BB_낱권ISBN"), strDmData);
			if( strDmData.IsEmpty() )
...
{
	CString strTmp = m_pDM_Book->GetCellData(_T("BB_DB_FLAG"), 1);
	if (m_nSelectedGrid == GRID_VOLUME || m_nSelectedGrid == GRID_VOLUME_HOLD)
	{
		...
		strTmp = m_pDM_Book->GetCellData(_T("BB_DB_FLAG"), 1);
...
INT CBOVolumeDlg::DisplayButtons()
{
	...
	nArrButtonCtrl[nButtonCount++] = IDC_bVOLUME_SAVE;
...
INT CBOVolumeDlg::DisplayContorls()
{
	...
	{
		GetDlgItem(IDC_bVOLUME_SAVE)->ShowWindow(SW_HIDE);
	}
	else
	{
		GetDlgItem(IDC_bVOLUME_SAVE)->ShowWindow(SW_SHOW);
	}
...
//	{
	...
	CString strTmp=_T("");
	for(INT i=0; i<strCopyCode.GetLength(); i++)
	{
		strTmp = strCopyCode.GetAt(i);
		if ( strTmp<_T("0") || strTmp>_T("9"))
		{
			AfxMessageBox(_T("복본기호 값이 잘못되었습니다. \n복본기호에는 숫자만 입력할 수 있습니다. 수정해주십시오."));
			GetDlgItem(IDC_eCOPY_CODE)->SetFocus();
			return;
		}
	}
	...
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eCOPY_CODE);
	if (pEdit == NULL) 
	{
		return;
	}
...
EFS_BEGIN
	...
	nArrOtherResources[nCount++] = IDC_bVOLUME_SAVE;
	nArrOtherResources[nCount++] = IDC_bDELETE;
	nArrOtherResources[nCount++] = IDC_bVOLUME_CLOSE;
...
INT CBOVolumeDlg::SaveVolume(CESL_ControlMgr * pCM)
{
	...
	if (IsDuplicateVolume(strModifiedVol, m_nSelectedRow))
	{
...
INT CBOVolumeDlg::SaveBook(CESL_ControlMgr * pCM)
{
	...
	INT nRegCodeLength;
	nRegCodeLength = m_pInfo->nREG_CODE_LENGTH;
	if(!strRegNo.IsEmpty() && strRegNo.Left(nRegCodeLength) != strModifiedRegCode) 
	{
		ESLAfxMessageBox(_T("등록번호가 부여된 자료는 등록구분을 수정할 수 없습니다."));
		GetDlgItem(IDC_eREG_CLASS_CODE)->SetWindowText(strRegNo.Left(nRegCodeLength));
		return 0;
	}
...
INT CBOVolumeDlg::InsertVolume()
{
	...
	strTmp = m_pDM_Book->GetCellData(_T("BB_DB_FLAG"), 0);
	strTmp = m_pDM_Book->GetCellData(_T("BB_DB_FLAG"), 1);
...
INT CBOVolumeDlg::InsertBook()
{
	...
	arrIgnorelist.Add(_T("BB_원부KEY"));
	arrIgnorelist.Add(_T("BB_수서정보KEY"));
	...
	if(strWorkingStatus.Left(3) == _T("BOL") && strWorkingStatus != _T("BOL111O"))
	{
		...
		m_pDM_Book->SetCellData(_T("BB_최초배가일자"), strTime, m_nSelectedRow);
		m_pDM_Book->SetCellData(_T("BB_정리완료일"), strTime, m_nSelectedRow);
		m_pDM_Book->SetCellData(_T("BB_검수일"), strTime, m_nSelectedRow);
		m_pDM_Book->SetCellData(_T("BB_배가인계일"), strTime, m_nSelectedRow);
		m_pDM_Book->SetCellData(_T("BB_등록일"), strTime, m_nSelectedRow);
		m_pDM_Book->SetCellData(_T("BB_등록인계일자"), strTime, m_nSelectedRow);	
	}
...
VOID CBOVolumeDlg::SetIME(INT nIDC, INT nMode)
{
	EFS_BEGIN
...
VOID CBOVolumeDlg::OnSetfocusePRICEOTHREINFO() 
{
	SetIME(IDC_ePRICE_OTHRE_INFO, 1);
}

VOID CBOVolumeDlg::OnSetfocusePHYSICALPROPERTIES() 
{
	SetIME(IDC_ePHYSICAL_PROPERTIES, 1);
}

VOID CBOVolumeDlg::OnSetfocuseAUTHOR() 
{
	SetIME(IDC_eAUTHOR, 1);
}

VOID CBOVolumeDlg::OnSetfocuseAPPENDIX() 
{
	SetIME(IDC_eAPPENDIX, 1);
}

VOID CBOVolumeDlg::OnSetfocusePAGECNT() 
{
	SetIME(IDC_ePAGE_CNT, 0);	
}

VOID CBOVolumeDlg::OnSetfocuseREGCLASSCODE() 
{
	SetIME(IDC_eREG_CLASS_CODE, 0);
}

VOID CBOVolumeDlg::OnSetfocuseSIZE() 
{
	SetIME(IDC_eSIZE, 0);
}

VOID CBOVolumeDlg::OnSetfocuseVOL() 
{
	SetIME(IDC_eVOL, 1);
}

VOID CBOVolumeDlg::OnSetfocuseVOLTITLE() 
{
	SetIME(IDC_eVOLTITLE, 1);
}

HBRUSH CBOVolumeDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{	
...
void CBOVolumeDlg::OnKillfocusedtVOLSORTNO() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_edtVOL_SORT_NO);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseVOL() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eVOL);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseVOLTITLE() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eVOLTITLE);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseAUTHOR() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eAUTHOR);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocusePUBLISHYEAR() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_ePUBLISH_YEAR);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseEAISBN() 
{
	CString strData;
...
void CBOVolumeDlg::OnKillfocuseISBN2() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eISBN2);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseISBN3() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eISBN3);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocusePHYSICALPROPERTIES() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_ePHYSICAL_PROPERTIES);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocuseAPPENDIX() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_eAPPENDIX);
	if (pEdit == NULL) 
	{
		return;
	}
...
void CBOVolumeDlg::OnKillfocusePRICEOTHREINFO() 
{
	CString strData;
	CEdit* pEdit = (CEdit*)GetDlgItem(IDC_ePRICE_OTHRE_INFO);
	if (pEdit == NULL) 
	{
		return;
	}
