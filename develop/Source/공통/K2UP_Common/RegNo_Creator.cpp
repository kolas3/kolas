...
//	);
	...
	if(tmpstr.IsEmpty())
	{
		...
		tmpquery.Format( _T("INSERT INTO CO_LAST_NUMBER_TBL ")
						 ...
						 _T("VALUES(%s, 'MO', '등록번호', '%s', '%s', %d, UDF_MANAGE_CODE);")
						 , tmpstr, regcode, result,	_ttoi((regno.Right(10).GetBuffer(0))) );
//		tmpquery.Format(
//			_T("INSERT INTO CO_LAST_NUMBER_TBL (REC_KEY, PUBLISH_FORM_CODE, KIND_CODE, DIVISION_VALUE, MAKE_YEAR, LAST_NUMBER) VALUES(%s, 'MO', '등록번호', '%s', '%s', %d);"),
//			tmpstr,
//			regcode,
//			result,
