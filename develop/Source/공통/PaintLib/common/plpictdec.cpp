...
/--------------------------------------------------------------------
|
|      $Id: plpictdec.cpp,v 1.1 2009/12/09 05:33:54 박종운 Exp $
    ...
    // data and then throw it away.
{
  ...
  try
  {
    for (i = 0; i < Height; i++)
    {
      ...
      wmemcpy (pLineArray[i], pRawLine, Width);
    }
  }
  catch (PLTextException)
  {
  ...
  try
  {
    ...
    else
    {
      ...
      { // For each line do...
            ...
            { // Packed data.
              ...
              if (pixelSize == 16)
              {
                ...
                { // Repeat the pixel len times.
                  wmemcpy (pBuf+(k*4*PixelPerRLEUnit), pBuf,
                          4*PixelPerRLEUnit);
                }
              ...
              else
              {
                ...
                { // Repeat the expanded byte len times.
                  wmemcpy (pBuf+(k*PixelPerRLEUnit), pBuf,
                          PixelPerRLEUnit);
                }
        ...
        pDestLine = pLineArray[i];
        if (pixelSize == 16)
          wmemcpy (pDestLine, pLineBuf, 4*Width);
        else
          wmemcpy (pDestLine, pLineBuf, Width);
      }
    }
  }
  catch (PLTextException)
    ...
    // Max. 8 bpp source format.
{
  ...
  {
    case 8:
      wmemcpy (pDestBuf, pSrcBuf, Width);
      break;
    case 4:
      for (i=0; i<Width; i++)
      {
...
|      *** empty log message ***
|
|      Revision 1.1  2008/02/29 11:02:01  PDJ
|      *** empty log message ***
|
