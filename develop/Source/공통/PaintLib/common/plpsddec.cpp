...
/--------------------------------------------------------------------
|
|      $Id: plpsddec.cpp,v 1.1 2009/12/09 05:33:56 박종운 Exp $
|
    ...
    // Creates a decoder
{
	...
	m_pLayerInfo = NULL;
	m_pMiscDataStart = NULL;
	m_pPal[0] = '\0';
	m_MiscDataSize = 0;
	m_NumLayers = 0;
	m_LayersRead = 0;
	m_Resolution = NULL;
	m_PSDHeader.Signature[0] = '\0';
	m_PSDHeader.Version = 0;
	m_PSDHeader.Reserved[0] = '\0';
	m_PSDHeader.Channels = 0;
	m_PSDHeader.Rows = 0;
	m_PSDHeader.Columns = 0;
	m_PSDHeader.BPP = 0;
	m_PSDHeader.Mode = 0;
}
...
{
  wmemcpy (pPSDHeader->Signature, pDataSrc->Read4Bytes (), 4);
  pPSDHeader->Version = ReadMWord (pDataSrc);
  wmemcpy (pPSDHeader->Reserved, pDataSrc->ReadNBytes (6), 6);
    ...
    )
{
  ...
  {
    char ResSignature[4];
    wmemcpy (ResSignature, pDataSrc->Read4Bytes (), 4);
    ...
    wmemcpy (pszName, pDataSrc->ReadNBytes(NameLen), NameLen);
    pszName[NameLen] = 0;
  ...
  if(m_MiscDataSize > 0)
  {
    ...
    if (LayerDataSize > 0)
    {
      ...
      for (int i=0; i<m_NumLayers; i++)
      {
        ...
        wmemcpy (pLayerInfo->BlendModeSig, pDataSrc->Read4Bytes (), 4);
        wmemcpy (pLayerInfo->BlendModeKey, pDataSrc->Read4Bytes (), 4);
    ...
    )
{
  for (int l=0; l<Channels; l++)
  {
    for (int y=0; y<Height; y++)
    {
      ...
      if (Channels == 1)
        wmemcpy (pLine, pDataSrc->ReadNBytes (Width), Width);
      else
      {
