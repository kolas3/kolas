...
BOOL CBOMarcEditDlg::OnInitDialog() 
{
	...
	m_marcEditor.AddLimitTag(_T("049"));
	Display();
	
	// 2005-01-12 적용버튼 숨긴다.
	// 적용이 화면에 적용이 아니라 마크 문법체크만 하므로 별 소용이 없기 때문.
	// 삭제해도 되지만 아직은 소스에 어떤 영행을 끼칠리 모르니까 그냥 숨겨놓기만 한다.
	(CButton*)GetDlgItem(IDC_bAPPLY_MARC)->ShowWindow(SW_HIDE);
...
BOOL CBOMarcEditDlg::PreTranslateMessage(MSG* pMsg) 
{
