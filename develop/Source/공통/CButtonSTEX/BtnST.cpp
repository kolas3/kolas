...
CButtonST::CButtonST()
{
	...
	// Default type is _T("flat") button
	m_bIsFlat = TRUE;
	// Button will be tracked also if when the window is inactive (like Internet Explorer)
	m_bAlwaysTrack = TRUE;
  
	// By default draw border in _T("flat") button 
	m_bDrawBorder = TRUE; 
  
	// By default icon is aligned horizontally
	m_byAlign = ST_ALIGN_HORIZ; 
  
	// By default, for _T("flat") button, don't draw the focus rect
...
VOID CButtonST::PreSubclassWindow() 
{
	...
	// (don't use the resource editor _T("Owner Draw") or
...
UINT CButtonST::OnGetDlgCode() 
{
	...
	// Tell the _tsystem if we want default state handling
...
{
	SetBkColor(pDC->m_hDC,RGB(244,244,244));
...
LRESULT CButtonST::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
	...
	// Let the _tsystem process changes to other style bits
...
VOID CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (sTitle.IsEmpty() == FALSE)
	{
		...
		// If button is pressed then _T("press") title also
...
//		pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
		BYTE TEXT_ALIGN_MODE = m_byTextAlign;
		pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | TEXT_ALIGN_MODE  | DT_VCENTER | DT_SINGLELINE );
//		pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK|TEXT_ALIGN_MODE  );
...
//			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | TEXT_ALIGN_MODE );
			pDC->DrawText(sTitle, -1, captionRect, DT_WORDBREAK | TEXT_ALIGN_MODE  | DT_VCENTER | DT_SINGLELINE );
//			pDC->DrawText(sTitle, -1, captionRect, TEXT_ALIGN_MODE );
			captionRect.OffsetRect(-1, -1);
...
//				Pass NULL to _tremove any icon from the button.
//		[IN]	nIconOut
...
//				Pass NULL to _tremove any bitmap from the button.
//		[IN]	crTransColorIn
...
DWORD CButtonST::SetAlignText(BYTE byTextAlign, BOOL bRepaint)
{
	switch (byTextAlign)
	{    
	case ST_ALIGN_HORIZ:
...
DWORD CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	...
	m_crColors[byColorIndex] = RGB(247,247,244);
...
//				Pass NULL to _tremove a previously loaded cursor.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	wmemset(m_szURL, 0, sizeof(m_szURL));
...
//				Pass NULL to _tremove any menu from the button.
//		[IN]	hParentWnd
...
//				This color will be used as the _T("transparent") color.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//				Pass NULL to _tremove any previously specified callback message.
//		[IN]	nMessage
...
//				Pass NULL to _tremove any previously specified sound.
//		[IN]	hMod
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	...
	if (m_bMouseOnButton || m_bIsPressed)
	{
		m_crColors[BTNST_COLOR_BK_IN] = RGB(247,247,244);
		crColor = m_crColors[BTNST_COLOR_BK_IN];
	}
	else
	{
		if (m_bIsFocused) {
			m_crColors[BTNST_COLOR_BK_FOCUS] = RGB(247,247,244);
			crColor = m_crColors[BTNST_COLOR_BK_FOCUS];
		}
		else {
			m_crColors[BTNST_COLOR_BK_OUT] = RGB(247,247,244);
