// Cat008FixedField_VM.cpp : implementation file
//

#include "stdafx.h"
#include "Cat008FixedField_VM.h"
...
/////////////////////////////////////////////////////////////////////////////
// CCat008FixedField_VM dialog


CCat008FixedField_VM::CCat008FixedField_VM(CESL_Mgr* pParent /*=NULL*/, CMarc *pMarc /*=NULL*/)
	: CCat008FixedFieldMgr(IDD, pParent)
{
	//{{AFX_DATA_INIT(CCat008FixedField_VM)
...
CCat008FixedField_VM::~CCat008FixedField_VM()
{
}

VOID CCat008FixedField_VM::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCat008FixedField_VM)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CCat008FixedField_VM, CDialog)
	//{{AFX_MSG_MAP(CCat008FixedField_VM)
...
/////////////////////////////////////////////////////////////////////////////
// CCat008FixedField_VM Operations

INT CCat008FixedField_VM::InitData()
{
	m_strGroupName = _T("[VM]");

	// Alias 세팅
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@0-5"),_T("입력날짜"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@6"),_T("유형"));
	...
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@15-17"),_T("발행국명"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@18-20"),_T("상영시간"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@22"),_T("이용대상수준"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@23"),_T("딸림자료1"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@24"),_T("딸림자료2"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@25"),_T("딸림자료3"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@26-27"),_T("대학부호"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@28"),_T("수정레코드"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@32"),_T("목록전거"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@33"),_T("자료유형"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@34"),_T("기술수준"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@35-37"),_T("언어구분"));
	m_mapMarcAlias_ControlAlias.SetAt(_T("008@38-39"),_T("정부기관"));

	// Control 세팅
	m_mapControlAlias_ItemName.SetAt(_T("유형"), _T("(발행년유형)"));
	m_mapControlAlias_ItemName.SetAt(_T("수정레코드"), _T("(수정레코드)"));
	m_mapControlAlias_ItemName.SetAt(_T("목록전거"), _T("(목록전거)"));
	m_mapControlAlias_ItemName.SetAt(_T("상영시간"), _T("(상영시간)"));
	m_mapControlAlias_ItemName.SetAt(_T("이용대상수준"), _T("(이용대상자수준)"));
	m_mapControlAlias_ItemName.SetAt(_T("딸림자료1"), _T("(008딸린자료)"));
	m_mapControlAlias_ItemName.SetAt(_T("딸림자료2"), _T("(008딸린자료)"));
	m_mapControlAlias_ItemName.SetAt(_T("딸림자료3"), _T("(008딸린자료)"));
	m_mapControlAlias_ItemName.SetAt(_T("자료유형"), _T("(시청각자료유형)"));
	...
	m_mapControlAlias_ItemName.SetAt(_T("발행국명"), _T("발행국부호"));
	m_mapControlAlias_ItemName.SetAt(_T("대학부호"), _T("한국대학부호"));
	m_mapControlAlias_ItemName.SetAt(_T("언어구분"), _T("언어"));
	m_mapControlAlias_ItemName.SetAt(_T("정부기관"), _T("정부기관부호"));
...
/////////////////////////////////////////////////////////////////////////////
// CCat008FixedField_VM message handlers

BOOL CCat008FixedField_VM::OnInitDialog() 
{
	...
	m_pCM = FindCM(_T("CM_BO_CAT_PROC_0324"));
	if (!m_pCM) goto ERR;
...
VOID CCat008FixedField_VM::OnOK() 
{
	...
	////////////////////////////////////////////////////////////////////////////////
	//18~20 자리가 3자리가 아닐경우 앞에서부터 0으로 채워줘야 한다.. ^^;;
	CString strItemData;
	ids = m_pInfo->pMarcMgr->GetItem(m_pMarc, _T("008@18-20"), strItemData);
	strItemData.TrimLeft();
	strItemData.TrimRight();
	for ( INT nLength = strItemData.GetLength() ; nLength < 3 ; nLength++ )
	{
		strItemData = _T("0") + strItemData ;
	}

	ids = m_pInfo->pMarcMgr->SetItem(m_pMarc, _T("008@18-20"), strItemData);
	////////////////////////////////////////////////////////////////////////////////
...
BOOL CCat008FixedField_VM::PreTranslateMessage(MSG* pMsg) 
{
	return CCat008FixedFieldMgr::PreTranslateMessage(pMsg);
}

VOID CCat008FixedField_VM::Onb007EDIT() 
{
...
HBRUSH CCat008FixedField_VM::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
