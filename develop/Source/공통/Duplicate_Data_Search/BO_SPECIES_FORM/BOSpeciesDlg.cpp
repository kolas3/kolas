...
VOID CBOSpeciesDlg::ShowCodeResource()
{
	...
	if (m_bShowCode)
		GetDlgItem(IDC_bSHOW_HIDE_CODE)->SetWindowText(_T("간략히◀◀"));
	else
		GetDlgItem(IDC_bSHOW_HIDE_CODE)->SetWindowText(_T("자세히▶▶"));
...
INT CBOSpeciesDlg::ValidateData()
{
	...
	// IBS_권서명 : IDC_eTITLE
	// IBS_저작자 : IDC_eAUTHOR1
	// 등록구분 : IDC_eREG_CLASS_CODE
	// BP_총책수 : 
	// BP_화폐구분 : IDC_cCURRENCY_CLASS
	// BP_가격 : IDC_ePRICE
	...
	INT arrResource[4] = {
		IDC_eTITLE, IDC_eAUTHOR1,
		IDC_eREG_CLASS_CODE, IDC_ePRICE
	};
...
INT CBOSpeciesDlg::ApplyIndexToMarc(INT idx /*= 0*/)
{
	...
	ids = m_pBOSpeciesApi->INDEX_to_CMARC(m_pDM_Index, &marc, 0);
	if (ids < 0) return ids;
	ids = m_pInfo->pMarcMgr->CheckSimpleSyntax(&marc);
	if (ids < 0) return ids;
...
INT CBOSpeciesDlg::ApplyCalculation()
{
	...
	if (strPriceType == _T("종가격"))
	{
		...
		ids = m_pFormApi->ApplySpeciesPriceToBookPrice(m_pDM_Book, dPrice);
		if (ids < 0) return -1;
	}
	// 가격 == 각각 책의 가격을 합한값
	else if (strPriceType == _T("권가격"))
	{
		...
		ids = m_pFormApi->GetBookPriceSum(m_pDM_Book, dPrice);
		if (ids < 0) return -1;
		nResult = ids;
	...
	dPrice = (dPrice * (100 - nDiscountRate)) / 100;

	// strTemp.Format(_T("%d"), nPrice);
	strTemp.Format(_T("%.2f"), dPrice);
...
INT CBOSpeciesDlg::MakeUpdateDM()
{
	...
	m_pDM_Purchase->RefreshDataManager(strOption);
	...
	m_pDM_Book->RefreshDataManager(strOption);
...
VOID CBOSpeciesDlg::OnbDEFAULTCONFIG() 
{
	if (SaveDefaultValue() < 0)
		...
		AfxMessageBox(_T("기본값 저장 성공"), MB_ICONINFORMATION);
}
...
VOID CBOSpeciesDlg::OnbSAVEAS() 
{
	...
	*/
	...
	{
		MessageBox(_T("저장 성공"), NULL, MB_OK | MB_ICONINFORMATION);	
...
VOID CBOSpeciesDlg::OnbSAVE() 
{
	...
	else if (GetOpenMode() == -1100)
	{
		...
		else
		{
			...
			MessageBox(_T("수정 성공"), NULL, MB_OK | MB_ICONINFORMATION);	
			m_bControlDisplay = TRUE;
			Init();
		}
	}
...
VOID CBOSpeciesDlg::OnbMARCIMPORT() 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		Display();
		AfxMessageBox(_T("Marc 반입 성공"), MB_ICONINFORMATION);
		return;
	}
...
VOID CBOSpeciesDlg::OnbDUPSEARCH() 
{
	...
	if (GetOpenMode() == -1100 && m_nParentSelectedRow >= 0)
		m_pDM_Parent->GetCellData(m_strSpeciesKeyAlias, m_nParentSelectedRow, m_strDupIndexKey);
	
	// 복본 API를 부른다.
	CDuplicateSearch_Interface impl(this);
	INT ids = impl.ShowWorkDuplicateSearch(_T("서명복본조사"), _T(""), _T("CM_단행_종_색인정보"), _T(""), m_strDupIndexKey);
	if (ids < 0)
	{
		MessageBox(_T("복본 자료가 없습니다."));
		return;
	}
...
VOID CBOSpeciesDlg::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent == 0)
	{
		...
		CDuplicateSearch_Interface impl(this);
		ids = impl.ShowOnlyTitleDuplicateSearch(_T("서명자동복본조사"), strTitle, strSearchCount, strSpeciesKey);
		if (ids < 0) return;
