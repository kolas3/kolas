...
INT CBOVolumeDlg::Init()
{
	...
	if (!GetDupIdxKey().IsEmpty()) 
	{
		...
		m_pDM_BookHold->RefreshDataManager(strOption);
	...
	else if (GetOpenMode() == 8000)
	{
		...
		INT ids = m_pDM_Book->RefreshDataManager(strOption);
		if (ids < 0)
			return -1;
...
INT CBOVolumeDlg::ApplyDeleteData()
{
	...
	if (GetOpenMode() == 1100 || GetOpenMode() == -1100 || GetOpenMode() == 2100 ||  GetOpenMode() == -2100)
	{
		...
		if (m_nSelectedGrid == GRID_VOLUME || m_nSelectedGrid == GRID_VOLUME_HOLD)
		{
			...
			if (nVolCount == 1) 
			{
				AfxMessageBox(_T("마지막 권이므로 삭제할 수 없습니다."));
				return -1;
			}
...
INT CBOVolumeDlg::DisplayContorls()
{
	...
	nArrVolumeOnlyCtrl[nVolumeOnlyCtrlCount++] = IDC_cSHELF_CODE;
...
INT CBOVolumeDlg::SortBookDM(CESL_DataMgr *pDM_Book)
{
	...
	for (INT idx = 0; idx < nRowCount; idx++)
	{
		...
		// 편권차를 가져온다.
		strVol = m_pDM_Book->GetCellData(_T("BB_편권차"), idx);
...
// 	strRegNoField = strRegCode + strRegNo;
	INT nRegCodeLength;
	nRegCodeLength = m_pParent->m_pInfo->nREG_CODE_LENGTH;
	strRegNoField = m_pParent->m_pInfo->MakeRegNo( strRegCode, strRegNo );
