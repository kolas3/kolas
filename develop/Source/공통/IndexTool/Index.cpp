...
INT  CIndex::RemoveSpecialChar(CString &strSrc)
{
	EFS_BEGIN	
    ...
    for( i = 0; i < strSrc.GetLength() ; i++ ){
		...
		for( j =0; j < EX_DEL_CHAR_LIST_CNT; j++){
            if( ( strSrc.GetAt(i) >= EX_DEL_CHAR_LISR[j].StartChar && strSrc.GetAt(i) <= EX_DEL_CHAR_LISR[j].LastChar ) ||
                ...
                if( ( f_Flag == FALSE && strSrc.GetAt(i) == _T('.') ) || ( f_Flag == FALSE && strSrc.GetAt(i) == _T(' ') ) || ( f_Flag == FALSE && strSrc.GetAt(i) == _T('"') ) ) continue;
                else break;
            }
