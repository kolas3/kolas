// DLG_006_RB.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDLG_006_RB dialog
CDLG_006_RB::CDLG_006_RB(CESL_Mgr* pParent /*=NULL*/, CMarc *pMarc /*=NULL*/)
	: CCat006UnionFixedFieldMgr(IDD, pParent)
{
	//{{AFX_DATA_INIT(CDLG_006_RB)
...
CDLG_006_RB::~CDLG_006_RB()
{
}
void CDLG_006_RB::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDLG_006_RB)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDLG_006_RB, CDialog)
	//{{AFX_MSG_MAP(CDLG_006_RB)
...
{	
	...
	m_mapControlAlias_ItemName.SetAt(_T("자료의형태")		, _T("(자료의형태)"));
	m_mapControlAlias_ItemName.SetAt(_T("자료의형태")		, _T("(자료의형태)"));
	m_mapControlAlias_ItemName.SetAt(_T("자료의형태")		, _T("(자료의형태)"));
...
BOOL CDLG_006_RB::OnInitDialog() 
{
	CDialog::OnInitDialog();

	if (InitESL_Mgr(_T("SM_FIXED_006_RB")) < 0)
	{
		AfxMessageBox(_T("InitESL_Mgr Error"));
		return FALSE;
	}

	m_pCM = FindCM(_T("CM_FIXED_006_RB"));
	if (!m_pCM) goto ERR;
...
BOOL CDLG_006_RB::PreTranslateMessage(MSG* pMsg) 
{
	return CCat006UnionFixedFieldMgr::PreTranslateMessage(pMsg);
}

BOOL CDLG_006_RB::Create(CWnd *pParentWnd)
{
	return CDialog::Create(IDD, pParentWnd);
}
HBRUSH CDLG_006_RB::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
