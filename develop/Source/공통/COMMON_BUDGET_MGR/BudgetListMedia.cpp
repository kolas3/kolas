...
BOOL CBudgetListMedia::OnInitDialog() 
{
	...
	if(InitESL_Mgr( _T("예산관리_예산편성_매체구분") )<0) {
		AfxMessageBox( _T("InitESL_Mgr Error : 예산관리_예산편성_매체구분") );
		return false;
	}
	
	
	m_pCM = FindCM(_T("CM_예산편성_매체구분"));
	m_pDM = FindDM(_T("DM_예산편성보조_매체구분"));
	m_pDM_EXE_SUB = FindDM(_T("DM_예산집행보조"));
	m_pGrid = (CESL_Grid*)m_pCM->FindControl(_T("그리드"));

	if(!m_pCM || !m_pDM || !m_pGrid || !m_pDM_EXE_SUB) {
		AfxMessageBox( _T("CM,DM,Grid Error : CM_예산편성_매체구분,DM_예산편성보조_매체구분,그리드,DM_예산집행보조") );
		return FALSE;
	}
...
INT CBudgetListMedia::ShowAllData()
{
	...
	{
		m_pDM->SetCellData( _T("UDF_매체구분"), m_pDM->GetCellData( _T("코드"), nIdx ) , nIdx );
...
VOID CBudgetListMedia::OnbtnBUDGETLISTMATINSERT() 
{
	...
	m_pCM->GetControlMgrData(_T("매체구분"),strCode,-1);
...
VOID CBudgetListMedia::OnbtnBUDGETLISTMATDELETE() 
{
	...
	CString strTmp;
	strTmp = m_pDM->GetCellData(_T("UDF_매체구분"),m_nCurrentIdx);
