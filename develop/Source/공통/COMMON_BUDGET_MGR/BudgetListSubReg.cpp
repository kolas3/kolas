// BudgetListSubReg.cpp : implementation file
//

#include "stdafx.h"
#include "BudgetListSubReg.h"
#include "BudgetApi.h"
...
/////////////////////////////////////////////////////////////////////////////
// CBudgetListSubReg dialog


CBudgetListSubReg::CBudgetListSubReg(CESL_Mgr* pParent /*=NULL*/)
	: CESL_Mgr(IDD, pParent)
{
	//{{AFX_DATA_INIT(CBudgetListSubReg)
...
CBudgetListSubReg::~CBudgetListSubReg()
{
}

BOOL CBudgetListSubReg::Create(CWnd *pParentWnd)
{
	return CDialog::Create(IDD, pParentWnd);
}


VOID CBudgetListSubReg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBudgetListSubReg)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CBudgetListSubReg, CDialog)
	//{{AFX_MSG_MAP(CBudgetListSubReg)
...
/////////////////////////////////////////////////////////////////////////////
// CBudgetListSubReg message handlers

BOOL CBudgetListSubReg::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	if(InitESL_Mgr( _T("예산관리_예산편성_보조등록구분") )<0) {
		AfxMessageBox( _T("InitESL_Mgr Error : 예산관리_예산편성_보조등록구분") );
		return false;
	}
	
	
	m_pCM = FindCM(_T("CM_예산편성_보조등록구분"));
	m_pDM = FindDM(_T("DM_예산편성보조_보조등록구분"));
	m_pDM_EXE_SUB = FindDM(_T("DM_예산집행보조"));
	m_pGrid = (CESL_Grid*)m_pCM->FindControl(_T("그리드"));

	if(!m_pCM || !m_pDM || !m_pGrid || !m_pDM_EXE_SUB) {
		AfxMessageBox( _T("CM,DM,Grid Error : CM_예산편성_보조등록구분,DM_예산편성보조_보조등록구분,그리드,DM_예산집행보조") );
		return FALSE;
	}
...
INT CBudgetListSubReg::GridDisplayEventProc(CString &strGridAlias, INT nRow, INT nCol, CString &strDMFieldAlias, CString &str)
{
...
INT CBudgetListSubReg::InitOpenMode()
{
...
INT CBudgetListSubReg::ShowAllData()
{
	CString sTemp;
	INT ii = 0;
	INT iTotal = 0;
	...
	{
		m_pDM->SetCellData( _T("UDF_보조등록구분"), m_pDM->GetCellData( _T("코드"), nIdx ) , nIdx );
	...
	if(iTotal != 0)
	{
		...
		SetDlgItemText(IDC_edtBUDGET_LIST_SUB_REG_ACCOUNT2,m_sBudgetTotal);
	}
	else
		SetDlgItemText(IDC_edtBUDGET_LIST_SUB_REG_ACCOUNT2,_T(""));
	m_pInfo->pCodeMgr->ConvertCodeToDescInDM(m_pDM,_T("보조등록구분"),_T("UDF_보조등록구분"));
...
VOID CBudgetListSubReg::OnbtnBUDGETLISTMATINSERT() 
{
	...
	m_pCM->GetControlMgrData(_T("보조등록구분"),strCode,-1);
...
VOID CBudgetListSubReg::OnbtnBUDGETLISTMATMODIFY() 
{
	// TODO: Add your control notification handler code here
...
VOID CBudgetListSubReg::OnbtnBUDGETLISTMATDELETE() 
{
	// TODO: Add your control notification handler code here
	// 2005-02-17 검색된 자료가 없으면 리턴한다.
	INT nCnt = m_pDM->GetRowCount();
	...
	CString strTmp;
	strTmp = m_pDM->GetCellData(_T("UDF_보조등록구분"),m_nCurrentIdx);
...
VOID CBudgetListSubReg::SetAllData(INT row, CString strUDF)
{
	const INT cnt = 2;
	CString alias[cnt];
	CString data[cnt];
	...
	m_pDM->SetCellData(_T("UDF_보조등록구분"),data[0],row);
	m_pDM->SetCellData(_T("예산할당액"),data[1],row);
	m_pDM->SetCellData(_T("UDF_상태"),strUDF,row);
	m_pDM->SetCellData(_T("보조유형"),_T("S"),row);

	m_nCurrentIdx =	row;
	ShowAllData();
}

INT CBudgetListSubReg::RefreshAll()
{
	...
	CString strWhere;
	strWhere.Format(_T("BUDGET_COMPILATION_KEY = %s AND CLASS_KIND = 'SUB' AND SUB_TYPE = 'S'"),m_strBUDGET_COMPILATION_KEY);
	...
	//예산집행 보조정보 적용 
	CBudgetApi api(this);
	api.ApplySubExecutionInfo(m_pDM,m_pDM_EXE_SUB,_T("S"));
...
BEGIN_EVENTSINK_MAP(CBudgetListSubReg, CDialog)
    //{{AFX_EVENTSINK_MAP(CBudgetListSubReg)
	ON_EVENT(CBudgetListSubReg, IDC_gridBUDGET_LIST_MAT, -600 /* Click */, OnClickgridBUDGETLISTMAT, VTS_NONE)
	ON_EVENT(CBudgetListSubReg, IDC_gridBUDGET_LIST_SUBREG, -600 /* Click */, OnClickgridBUDGETLISTSUBREG, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

VOID CBudgetListSubReg::OnClickgridBUDGETLISTMAT() 
{
	// TODO: Add your control notification handler code here
	
}

VOID CBudgetListSubReg::OnClickgridBUDGETLISTSUBREG() 
{
...
HBRUSH CBudgetListSubReg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
