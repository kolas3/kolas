// BudgetListManage1.cpp : implementation file
//

#include "stdafx.h"
#include "BudgetListManage1.h"
#include "BudgetApi.h"
...
/////////////////////////////////////////////////////////////////////////////
// CBudgetListManage dialog


CBudgetListManage::CBudgetListManage(CESL_Mgr* pParent /*=NULL*/)
	: CESL_Mgr(IDD, pParent)
{
	m_sBudgetTotal = _T("");
	//{{AFX_DATA_INIT(CBudgetListManage)
...
CBudgetListManage::~CBudgetListManage()
{
}

BOOL CBudgetListManage::Create(CWnd *pParentWnd)
{
	return CDialog::Create(IDD, pParentWnd);
}

VOID CBudgetListManage::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBudgetListManage)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CBudgetListManage, CDialog)
	//{{AFX_MSG_MAP(CBudgetListManage)
	ON_WM_CTLCOLOR()
...
/////////////////////////////////////////////////////////////////////////////
// CBudgetListManage message handlers

BEGIN_EVENTSINK_MAP(CBudgetListManage, CDialog)
    //{{AFX_EVENTSINK_MAP(CBudgetListManage)
	ON_EVENT(CBudgetListManage, IDC_gridBUDGET_LIST_MANAGE, -600 /* Click */, OnClickgridBUDGETLISTMANAGE, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

VOID CBudgetListManage::OnClickgridBUDGETLISTMANAGE() 
{
...
BOOL CBudgetListManage::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	// TODO: Add extra initialization here
	if(InitESL_Mgr( _T("예산관리_예산편성_관리구분") )<0) {
		AfxMessageBox( _T("InitESL_Mgr Error : 예산관리_예산편성_관리구분") );
		return false;
	}
	
	
	m_pCM = FindCM(_T("CM_예산편성_관리구분"));
	m_pDM = FindDM(_T("DM_예산편성보조_관리구분"));
	m_pDM_EXE_SUB = FindDM(_T("DM_예산집행보조"));
	m_pGrid = (CESL_Grid*)m_pCM->FindControl(_T("그리드"));

	if(!m_pCM || !m_pDM || !m_pGrid || !m_pDM_EXE_SUB) {
		AfxMessageBox( _T("CM,DM,Grid Error : CM_예산편성_관리구분,DM_예산편성보조_관리구분,그리드,DM_예산집행보조") );
		return FALSE;
	}
...
INT CBudgetListManage::GridDisplayEventProc(CString &strGridAlias, INT nRow, INT nCol, CString &strDMFieldAlias, CString &str)
{
...
INT CBudgetListManage::InitOpenMode()
{
	if( m_nOpenMode<0 )
	{
...
INT CBudgetListManage::ShowAllData()
{
	CString sTemp;
	INT ii = 0;
	INT iTotal = 0;
	...
	{
		m_pDM->SetCellData( _T("UDF_관리구분"), m_pDM->GetCellData( _T("코드"), nIdx ) , nIdx );
	...
	if(iTotal != 0)
	{
		...
		SetDlgItemText(IDC_edtBUDGET_LIST_MANAGE_ACCOUNT2,m_sBudgetTotal);
	}
...
VOID CBudgetListManage::OnbtnBUDGETLISTMATINSERT() 
{
	...
	m_pCM->GetControlMgrData(_T("관리구분"),strCode,-1);
...
VOID CBudgetListManage::OnbtnBUDGETLISTMATMODIFY() 
{
	// TODO: Add your control notification handler code here
...
VOID CBudgetListManage::OnbtnBUDGETLISTMATDELETE() 
{
	// TODO: Add your control notification handler code here
	// 2005-02-17 검색된 자료가 없으면 리턴한다.
	INT nCnt = m_pDM->GetRowCount();
...
VOID CBudgetListManage::SetAllData(INT row, CString strUDF)
{
	const INT cnt = 2;
	CString alias[cnt];
	CString data[cnt];
	...
	m_pDM->SetCellData(_T("UDF_관리구분"),data[0],row);
	m_pDM->SetCellData(_T("예산할당액"),data[1],row);
	m_pDM->SetCellData(_T("UDF_상태"),strUDF,row);
	m_pDM->SetCellData(_T("보조유형"),_T("A"),row);

	m_nCurrentIdx =	row;
	ShowAllData();
}

INT CBudgetListManage::RefreshAll()
{
	...
	CString strWhere;
	strWhere.Format(_T("BUDGET_COMPILATION_KEY = %s AND CLASS_KIND = 'MAN' AND SUB_TYPE = 'A'"),m_strBUDGET_COMPILATION_KEY);
	...
	//예산집행 보조정보 적용 
	CBudgetApi api(this);
	api.ApplySubExecutionInfo(m_pDM,m_pDM_EXE_SUB,_T("A"));

	ShowAllData();
//	OnClickgridBUDGETLISTMANAGE();

	return 0;

}
HBRUSH CBudgetListManage::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
