// BudgetListMat.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CBudgetListMat dialog


CBudgetListMat::CBudgetListMat(CESL_Mgr* pParent /*=NULL*/)
	: CESL_Mgr(IDD, pParent)
{
	//{{AFX_DATA_INIT(CBudgetListMat)
...
CBudgetListMat::~CBudgetListMat()
{
}

VOID CBudgetListMat::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBudgetListMat)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CBudgetListMat, CDialog)
	//{{AFX_MSG_MAP(CBudgetListMat)
...
BOOL CBudgetListMat::Create(CWnd *pParentWnd)
{
	return CDialog::Create(IDD, pParentWnd);
}

/////////////////////////////////////////////////////////////////////////////
// CBudgetListMat message handlers

BOOL CBudgetListMat::OnInitDialog() 
{
...
INT CBudgetListMat::GridDisplayEventProc(CString &strGridAlias, INT nRow, INT nCol, CString &strDMFieldAlias, CString &str)
{
...
INT CBudgetListMat::InitOpenMode()
{
...
BEGIN_EVENTSINK_MAP(CBudgetListMat, CDialog)
    //{{AFX_EVENTSINK_MAP(CBudgetListMat)
	ON_EVENT(CBudgetListMat, IDC_gridBUDGET_LIST_MAT, -600 /* Click */, OnClickgridBUDGETLISTMAT, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

VOID CBudgetListMat::OnClickgridBUDGETLISTMAT() 
{
...
INT CBudgetListMat::ShowAllData()
{
	CString sTemp;
	INT ii = 0;
	INT iTotal = 0;
	...
	{
		m_pDM->SetCellData( _T("UDF_자료구분"), m_pDM->GetCellData( _T("코드"), nIdx ) , nIdx );
	...
	if(iTotal != 0)
	{
		...
		SetDlgItemText(IDC_edtBUDGET_LIST_MAT_ACCOUNT2,m_sBudgetTotal);
	}
	else
		SetDlgItemText(IDC_edtBUDGET_LIST_MAT_ACCOUNT2,_T(""));
...
VOID CBudgetListMat::OnbtnBUDGETLISTMATINSERT() 
{
	// TODO: Add your control notification handler code here
...
VOID CBudgetListMat::OnbtnBUDGETLISTMATMODIFY() 
{
	// TODO: Add your control notification handler code here
...
VOID CBudgetListMat::OnbtnBUDGETLISTMATDELETE() 
{
	// TODO: Add your control notification handler code here
	// 2005-02-17 검색된 자료가 없으면 리턴한다.
	INT nCnt = m_pDM->GetRowCount();
...
VOID CBudgetListMat::SetAllData(INT row, CString strUDF)
{
	const INT cnt = 2;
	CString alias[cnt];
	CString data[cnt];
	...
	m_pDM->SetCellData(_T("UDF_자료구분"),data[0],row);
	m_pDM->SetCellData(_T("예산할당액"),data[1],row);
	m_pDM->SetCellData(_T("UDF_상태"),strUDF,row);
	m_pDM->SetCellData(_T("보조유형"),_T("M"),row);

	m_nCurrentIdx =	row;
	ShowAllData();
}

INT CBudgetListMat::RefreshAll()
{
	...
	CString strWhere;
	strWhere.Format(_T("BUDGET_COMPILATION_KEY = %s AND CLASS_KIND = 'MAT' AND SUB_TYPE = 'M'"),m_strBUDGET_COMPILATION_KEY);
...
HBRUSH CBudgetListMat::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
