...
BOOL CBudgetListDept::OnInitDialog() 
{
	...
	if(InitESL_Mgr( _T("예산관리_예산편성_학부별") )<0) {
		AfxMessageBox( _T("InitESL_Mgr Error : 예산관리_예산편성_학부별") );
		return false;
	}
	
	
	m_pCM = FindCM(_T("CM_예산편성_학부별"));
	m_pDM = FindDM(_T("DM_예산편성보조_학부별"));
	m_pDM_EXE_SUB = FindDM(_T("DM_예산집행보조"));
	m_pGrid = (CESL_Grid*)m_pCM->FindControl(_T("그리드"));

	if(!m_pCM || !m_pDM || !m_pGrid || !m_pDM_EXE_SUB) {
		AfxMessageBox( _T("CM,DM,Grid Error : CM_예산편성_학부별,DM_예산편성보조_학부별,그리드") );
		return FALSE;
	}
...
INT CBudgetListDept::ShowAllData()
{
	...
	{
		m_pDM->SetCellData( _T("UDF_학부"), m_pDM->GetCellData( _T("코드"), nIdx ) , nIdx );
	...
	m_pInfo->pCodeMgr->ConvertCodeToDescInDM(m_pDM,_T("학부"),_T("UDF_학부"));
...
VOID CBudgetListDept::OnbtnBUDGETLISTCOURCEINSERT() 
{
	...
	m_pCM->GetControlMgrData(_T("학부별"),strCode,-1);
...
VOID CBudgetListDept::OnbtnBUDGETLISTCOURCEDELETE() 
{
	...
	CString strTmp;
	strTmp = m_pDM->GetCellData(_T("UDF_학부"),m_nCurrentIdx);

	CString msg;
	msg.Format(_T(" [%s] 학처의  예산할당을 삭제하겠습니까?"), strTmp);
