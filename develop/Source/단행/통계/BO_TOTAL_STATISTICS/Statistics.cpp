...
VOID CStatistics::ShowConditionDlg()
{
	...
	//===================================================
	//2009.04.08 ADD BY PJW : 제적자료포함유무를 값을 설정한다.
	CString strRemoveBookBringYN;
	CString strRemoveBookBringYNQuery;

	strRemoveBookBringYNQuery.Format(_T("SELECT VALUE_NAME FROM ESL_MANAGE_TBL WHERE GROUP_1='L' AND GROUP_2='기타' AND GROUP_3='공통' AND CLASS_ALIAS='자산통계 제적자료포함' AND MANAGE_CODE = UDF_MANAGE_CODE"));
...
VOID CStatistics::PrintStatistics()
{
	...
	m_pPrintMgr->SetSIReportFilename(_T("K2UP_자산통계자료"));
	m_pPrintMgr->Print();
}
...
VOID CStatistics::ChangeColumnHeaderOfSubjectCode()
{
	...
	if( strResult.IsEmpty() == FALSE )
	{
		...
		m_pGrid->SetTextMatrix(0, idx+3, _T("계") );
		m_pGrid->SetTextMatrix(0, idx+4, _T("가격합계") );
	}
}
