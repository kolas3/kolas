// BO_ACQ_IMPORT_DATA_MANAGER.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE BO_ACQ_IMPORT_DATA_MANAGERDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("BO_ACQ_IMPORT_DATA_MANAGER.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(BO_ACQ_IMPORT_DATA_MANAGERDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(BO_ACQ_IMPORT_DATA_MANAGERDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("BO_ACQ_IMPORT_DATA_MANAGER.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(BO_ACQ_IMPORT_DATA_MANAGERDLL);
	}
	return 1;   // ok
}
...
{
	CBO_ACQ_ImportDataMgr * m_pWindow = new CBO_ACQ_ImportDataMgr(NULL);
...
{
	CBO_ACQ_ImportDataMgr * m_pWindow = NULL;
	if (!pWindow) return NULL;
	else
		 m_pWindow = (CBO_ACQ_ImportDataMgr*)pWindow;
	...
	{
		m_pWindow->ShowImportDataDlg(SUPDATE);
		return NULL;
	}
	else if(strButtonAlias == _T("삭제"))
	{
		m_pWindow->DeleteImportData();
		return NULL;
	}
	else if(strButtonAlias == _T("구입처리"))
	{
		m_pWindow->PurchaseImportData();
		return NULL;
	}
	else if(strButtonAlias == _T("기증처리"))
	{
		m_pWindow->DonateImportData();
		return NULL;
	}
	...
	//==============================================
	//20080618 ADD BY PJW : 반입자료관리에 서지정보 입력 추가
	else if(strButtonAlias == _T("입력"))
	{
		m_pWindow->ShowImportDataDlg(SINSERT);
		return NULL;
	}
	//==============================================
	return NULL;
