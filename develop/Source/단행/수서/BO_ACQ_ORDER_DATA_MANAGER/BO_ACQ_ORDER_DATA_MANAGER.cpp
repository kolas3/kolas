...
__declspec(dllexport) CESL_Mgr* CreateModelessDlg(CString strSM, CString strButtonAlias, BOOL bReadOnly, CWnd *pWindow)
{
	...
	if(strButtonAlias == _T("검색"))
	{
		...
		//=============================================
		//20080608 UPDATE BY PJW : 검수자료관리에서 검색창을 띄우면 에러나서 수정 
		return NULL;
	...
	else if(strButtonAlias == _T("검수완료"))
	{
		m_pWindow->CheckInComplete();
		return NULL;
	}
	// 2004.10.29 자료반출, 파일반입 추가
	...
	else if(strButtonAlias == _T("검수완료자료"))
	{
		m_pWindow->ViewCheckInCompleteData();
		return NULL;
	}
	//JOB.2019.0073 : 충돌 License 제거
// 	else if(strButtonAlias == _T("주문메일"))
// 	{
