...
INT CDBProc::MakeDMFromDB( CString sBookDMAlias , CString sWhere , CBLMapData_Mem* pMapData , CESL_DataMgr*		pDM_Other )
{
	...
	if( ids < 0 ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 8 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 

	SEDM.TBL_NAME = _T("SE_BOOK_TBL BB , IDX_SE_TBL IB , SE_VOL_TBL V");	
	
	//2020.03.27 JOB.2020.0021 add by kyj : OF 온라인 도서 제외인데 거의 사용하지 않는 메체구분이여서 제외
	//SEDM.CONDITION = _T("BB.VOL_KEY=V.REC_KEY AND BB.SPECIES_KEY=IB.REC_KEY AND BB.BOOK_TYPE IN ('B','C') ");	
	CESL_ControlMgr* pCM = m_pParentMgr->FindCM(_T("CM_BO_LOC_2100"));
	if( pCM == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 9 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 
	
	CESL_Control_Element* pIncludeAppendix = pCM->FindControlElement( _T("부록포함체크") );
	if( pIncludeAppendix == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 10 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 
	...
	if( btn1 == NULL ) {
		...
		if( btn1 == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 11 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 
	}
	
	
	if( btn1->GetCheck() == TRUE ) 
	{
		SEDM.CONDITION = _T("BB.VOL_KEY=V.REC_KEY AND BB.SPECIES_KEY=IB.REC_KEY AND BB.BOOK_TYPE IN ('B','C','A') ");
	}
	else
	{	SEDM.CONDITION = _T("BB.VOL_KEY=V.REC_KEY AND BB.SPECIES_KEY=IB.REC_KEY AND BB.BOOK_TYPE IN ('B','C') ");
	...
	//sWhere2.Format( _T("%s AND BB.MEDIA_CODE <> 'OF' "),sWhere);
 	if ( btn1->GetCheck() == FALSE ) 
	{
 		sWhere2.Format( _T("%s AND BB.BOOK_APPENDIX_FLAG = 'B' "),sWhere);
 	}
	else {
		sWhere2.Format( _T("%s "), sWhere);
	}
...
INT CDBProc::UpdateLostBookWorkingStatus( CESL_DataMgr *pDM , CString sShelfLocCode , CString sChangeWorkingStatus )
{
	...
	//2020.03.27 JOB.2020.0021 add by kyj : OF 온라인 도서 제외인데 거의 사용하지 않는 메체구분이여서 제외
	CESL_ControlMgr* pCM = m_pParentMgr->FindCM(_T("CM_BO_LOC_2100"));
	if( pCM == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 3 , _T("UpdateLostBookWorkingStatus") ); 
	
	CESL_Control_Element* pIncludeAppendix = pCM->FindControlElement( _T("부록포함체크") );
	if( pIncludeAppendix == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 4 , _T("UpdateLostBookWorkingStatus") ); 
	
	CButton* btn1 = (CButton*)(m_pParentMgr->GetDlgItem( pIncludeAppendix->CTRL_ID ));
	if( btn1 == NULL ) {
		btn1 = (CButton*)(m_pParentMgr->pParentMgr->GetDlgItem( pIncludeAppendix->CTRL_ID ));
		if( btn1 == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 5 , _T("UpdateLostBookWorkingStatus") ); 
	}
	if( btn1->GetCheck() == TRUE ) {
		sDefaultSql.Replace(_T("AND BOOK_APPENDIX_FLAG='B'"),_T(""));
	}
	//==============================================================================================================
...
INT CDBProc::GetDBForFile( CESL_DataMgr *pDM , INT nMode )
{
	...
	//2020.04.21 JOB.2020.0021 add by kyj
	CESL_ControlMgr* pCM = m_pParentMgr->FindCM(_T("CM_BO_LOC_2100"));
	if( pCM == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 9 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 	
	CESL_Control_Element* pIncludeAppendix = pCM->FindControlElement( _T("부록포함체크") );
	if( pIncludeAppendix == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 10 , _T("MakeDMFromDB( CString sBookDMAlias , CString sWhere )") ); 	
	...
	if (btn1->GetCheck() == FALSE )
	{
		pDM->CONDITION = _T("BOOK_APPENDIX_FLAG = 'B' ");
	}
	else
	{
		pDM->CONDITION = _T("");
	...
	{
		SEDM.CONDITION =  _T("BOOK_TYPE IN ( 'B' , 'C' , 'A' )");
	}
...
INT CDBProc::EndBookCheckDBProc( CESL_DataMgr *pDM , CESL_DataMgr *pDM2 , CString sCheckNoKey , CString sShelfLocCode )
{
	...
	//2020.03.27 JOB.2020.0021 add by KYJ
	CESL_ControlMgr* pCM = m_pParentMgr->FindCM(_T("CM_BO_LOC_2100"));
	if( pCM == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 3 , _T("UpdateLostBookWorkingStatus") ); 	
	CESL_Control_Element* pIncludeAppendix = pCM->FindControlElement( _T("부록포함체크") );
	if( pIncludeAppendix == NULL ) ERROR_MSG_RETURN_ERROR_CODE( m_pParentMgr->m_hWnd , - 4 , _T("UpdateLostBookWorkingStatus") ); 
...
//		sWhere += _T(" AND SHELF_LOC_CODE IN (")+sShelfLocCode+_T(") ");
	...
	//2020.03.27 JOB.2020.0021 add by KYJ
	if( btn1->GetCheck() == FALSE ) {
		sWhere += _T(" AND BOOK_APPENDIX_FLAG = 'B' ");
	}
...
//		sWhere += _T(" AND B.SHELF_LOC_CODE IN (")+sShelfLocCode+_T(") AND BOOK_TYPE IN ('A','B','C')");
		//2020.03.27 JOB.2020.0021 EDIT by KYJ
		//sWhere += _T(" AND B.SHELF_LOC_CODE IN ( '")+sShelfLocCode+_T("' ) AND BOOK_TYPE IN ('A','B','C')");
		sWhere += _T(" AND B.SHELF_LOC_CODE IN ( '")+sShelfLocCode+_T("' )");
		//===================================================		
	}
	//2020.03.27 JOB.2020.0021 EDIT by KYJ
	if( btn1->GetCheck() == TRUE ) {
		sWhere += _T(" AND BOOK_TYPE IN ('A','B','C') ");
	}
	else
	{
		sWhere += _T(" AND BOOK_TYPE IN ('B','C') ");
	}
