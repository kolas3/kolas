...
CSIReportPrintObjectMgr::CSIReportPrintObjectMgr()
{
	...
	m_bFrawOuterLine = FALSE;		// 기본적으로 외곽의 사각형은 그리지 않는다.
	
	// 바코드 관련 값
	m_strBarcodeStyle = _T("6");
	m_nBarcodeWidthGap = 5;		// 바코드 양쪽 여백
	m_nBarcodeHeight = 30;		// 바코드 높이
...
INT CSIReportPrintObjectMgr::Print_AddCard(INT nIdx, INT nRPos, INT nCPos, CSIReportPrintObject* pData)
{
	...
	if (m_bFrawOuterLine == TRUE) {			// 외곽의 사각형을 그릴경우
...
INT CSIReportPrintObjectMgr::SetFont(INT nFontHeight, CString strFontFace)
{
