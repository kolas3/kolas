...
#define TIMER_HIDE				4000		// Hide timer (ms)
...
void CXInfoTip::OnPaint() 
{
	...
	// Create the brushes
	InnerFrameBrush.CreateSolidBrush(::GetSysColor(COLOR_SCROLLBAR));	//옆 테두리색 부분
	FrameBrush.CreateSolidBrush(::GetSysColor(COLOR_WINDOWTEXT));		//글자색 부분
	WindowBrush.CreateSolidBrush(::GetSysColor(COLOR_WINDOW));			//안에 채워지는 색
...
//마우스가 움직임이나 마우스 키가 눌려졌을때 말풍선을 사라지게 하려면 주석을 없애면 됨..
...
void CXInfoTip::OnLButtonDown(UINT nFlags, CPoint point) 
{
	Hide();
	CWnd::OnLButtonDown(nFlags, point);
}

void CXInfoTip::OnRButtonDown(UINT nFlags, CPoint point) 
{
	Hide();	
