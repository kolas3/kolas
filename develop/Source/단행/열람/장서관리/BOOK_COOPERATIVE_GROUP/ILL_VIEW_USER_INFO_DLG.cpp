...
CILL_VIEW_USER_INFO_DLG::CILL_VIEW_USER_INFO_DLG(CESL_Mgr* pParent /*=NULL*/)
...
CILL_VIEW_USER_INFO_DLG::~CILL_VIEW_USER_INFO_DLG()
{
}

void CILL_VIEW_USER_INFO_DLG::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CILL_VIEW_USER_INFO_DLG, CDialog)
	//{{AFX_MSG_MAP(CILL_VIEW_USER_INFO_DLG)
...
int CILL_VIEW_USER_INFO_DLG::Init_Grid()
{
...
VOID CILL_VIEW_USER_INFO_DLG::SetGridCellColor(CESL_Grid *pGrid, INT nRow, INT nCol, COLORREF bk, COLORREF fk)
{
	if ( pGrid )
	{
		pGrid->SetRow(nRow);
...
BOOL CILL_VIEW_USER_INFO_DLG::OnInitDialog() 
{
	...
	mCtrl_pGrid->SubclassDlgItem(IDC_grid_ILL_USER_VIEW_INFO, this);
	...
	{
		mCtrl_pGrid->m_uUseKeyFlag = ESL_GRID_NOT_USE_F12;
	}	
	else
	{
		mCtrl_pGrid->m_bPrivacyPrintLogRecord = TRUE;
		mCtrl_pGrid->m_strPrintInfo = _T("책두레 이용자 상세보기");
	}

	
	INT ids;
	ids = m_pLoanMangeValue->GetManageValue( _T("열람"), _T("공통"), _T("이용자관리"), _T("대출화면주민번호표시유무"), m_strUserNoHideYN );
	if( 0 > ids )
	{
		return FALSE;
	}
	m_strUserNoHideYN.TrimLeft();m_strUserNoHideYN.TrimRight();
	m_strUserNoHideYN.MakeUpper();
...
int CILL_VIEW_USER_INFO_DLG::ResizeControl()
{
...
void CILL_VIEW_USER_INFO_DLG::OnOk() 
{
	
	CDialog::OnOK();
}

BOOL CILL_VIEW_USER_INFO_DLG::PreTranslateMessage(MSG* pMsg) 
{
...
int CILL_VIEW_USER_INFO_DLG::SetData(int nIdx)
{
	...
	ids = m_pParentDM->GetCellData(_T("이용자KEY"), nIdx, strGet);
	...
	CString strQuery = _T("");
	strQuery.Format(
		_T("SELECT CLU.NAME, CLU.USER_NO, CLU.E_MAIL, CLU.H_PHONE, CLU.HANDPHONE,  ")
		_T("DECODE(CLU.USER_CLASS, '0', '정상회원', '1', '대출정지회원', '2', '제적회원', '3', '대출중분실회원', '미정의') AS UCLASS, ")
		_T("(SELECT C.LIB_NAME FROM MN_LIBINFO2_TBL C WHERE C.MANAGE_CODE = CLU.MANAGE_CODE AND ROWNUM = 1) AS RLIBNAME, ")
		_T("CLU.ILL_APL_STOP_DATE, ")
		_T("CLU.ILL_EXPIRE_RETURN_CNT ")
		_T("FROM CO_LOAN_USER_TBL CLU ")

		_T("WHERE CLU.REC_KEY = %s"),
		strGet
		);
	if(FALSE == strGet.IsEmpty())
	{
	...
	if( _T("Y") != m_strUserNoHideYN )
	{
		CString strUserNo;
		ids = TmpDM.GetCellData(0, 1, strUserNo);
		if( 0 > ids ) return -1;
...
void CILL_VIEW_USER_INFO_DLG::OnbtnPREV() 
{ 
...
void CILL_VIEW_USER_INFO_DLG::OnbtnNEXT() 
{
...
VOID CILL_VIEW_USER_INFO_DLG::OnClickedCaptionButton(BOOL KOLAS1004_SECURITY_STATE)
{
EFS_BEGIN
