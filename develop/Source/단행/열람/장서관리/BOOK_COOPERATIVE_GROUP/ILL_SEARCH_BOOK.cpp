...
BOOL CILL_SEARCH_BOOK::OnInitDialog() 
{
	...
	CString strQuery;
	strQuery.Format(_T("SELECT VALUE_1 FROM ESL_MANAGE_TBL WHERE VALUE_NAME = '책두레사용여부' AND MANAGE_CODE = '%s'"), m_pInfo->MANAGE_CODE);
	CESL_DataMgr pDM;
...

	{
		...
		if(IDOK == Dlg.DoModal())
		{
			...
			if(0 == m_nSearchMode)
			{
				...
				ids = pBookDM->GetCellData(_T("부록자료"), nIdx, m_strAppendixFlag);
			}
			else
			{
				...
				ids = pBookDM->GetCellData(_T("등록번호"), nIdx, m_strSelectedBookRegNo);
...
INT CILL_SEARCH_BOOK::GetSqlSearchBookInfo( CString &sWhere , INT nMode )
{
	...
	if(pChkILLUse->GetCheck() == 1 && 0 == nMode)
	{
		...
		GetControlData(sCMAlias, _T("관리구분"), strTargetManageCode);
		CESL_DataMgr *pDM = FindDM(_T("DM_USERMAIN_DETAIL_SEARCH_ILL_MANAGE_CODE_LIST"));
		if(strTargetManageCode.IsEmpty() || _T("0") == strTargetManageCode || _T("적용안함") == strTargetManageCode)
		{
			
			pDM->RefreshDataManager(_T("EMT.VALUE_NAME = '책두레사용여부' AND EMT.VALUE_1 = 'Y'"));
			for(INT i=0; i<pDM->GetRowCount(); i++)
			{
				strAddQuery = _T("");
				strTargetManageCode = pDM->GetCellData(_T("관리구분"), i);
				strRegCodeList = pDM->GetCellData(_T("책두레허용등록구분목록"), i);
				strSeparateCodeList = pDM->GetCellData(_T("책두레허용별치기호목록"), i);
				strLocCodeList = pDM->GetCellData(_T("책두레허용자료실목록"), i);
		...
		{			
			...
			strQuery.Format(_T("SELECT VALUE_1 FROM ILL_MANAGE_TBL WHERE MANAGE_CODE = '%s' AND VALUE_NAME = '책두레허용등록구분목록'"), strTargetManageCode);
			...
			strQuery.Format(_T("SELECT VALUE_1 FROM ILL_MANAGE_TBL WHERE MANAGE_CODE = '%s' AND VALUE_NAME = '책두레허용자료실목록'"), strTargetManageCode);
			...
			strQuery.Format(_T("SELECT VALUE_1 FROM ILL_MANAGE_TBL WHERE MANAGE_CODE = '%s' AND VALUE_NAME = '책두레허용별치기호목록'"), strTargetManageCode);
