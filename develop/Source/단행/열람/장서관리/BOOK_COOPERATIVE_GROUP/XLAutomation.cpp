...
BOOL CXLAutomation::SetActiveWorksheet(int nWorksheet)
{
	...
	m_pdispWorksheet = varg1.pdispVal;
...
//++2009.12.17 ADD BY CJY {{++
// GetWorksheetName 함수와 SetCellsValueToString 함수를 토대로 작성
// 제작자가 만든 함수가 아니기 때문에 검증되지 않은 오류가 있을수 있음
BOOL CXLAutomation::SetWorksheetName(int nWorksheet, CString szStr)
{
	...
	ClearAllArgs();
    AddArgumentCString(NULL, 0, szStr );
	if (!ExlInvoke(varg1.pdispVal, _T("Name"), NULL, DISPATCH_PROPERTYPUT, 0))
		return FALSE;
	ReleaseVariant(&varg1);
