...
int CILL_VIEW_BOOK_INFO_DLG::SetData(int nIdx)
{
	...
	m_pParentDM->GetCellData(_T("트랜잭션번호"), nIdx, m_strTransNo);
	m_pParentDM->GetCellData(_T("책KEY"), nIdx, m_strBookKey);
...
int CILL_VIEW_BOOK_INFO_DLG::Show_BtnChangeRegNo()
{
	...
	ids = m_pParentDM->GetCellData(
		_T("진행상태"), m_pParentGrid->SelectGetIdx(), strState);
	if(0 > ids) return ids;

	CString strGiveManageCode;
	ids = m_pParentDM->GetCellData(
		_T("제공도서관_관리구분"), m_pParentGrid->SelectGetIdx(), strGiveManageCode);
	if(0 > ids) return ids;
	
	
	
 	UINT bShowFlag = SW_SHOW;
 	if(_T("요청된자료") == strState || _T("0014") == strState)
 	{
 		bShowFlag = SW_SHOW;
 	}
