// BO_CAT_REGNO_GIVE_LOST.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CBO_CAT_REGNO_GIVE_LOST dialog


CBO_CAT_REGNO_GIVE_LOST::CBO_CAT_REGNO_GIVE_LOST(CESL_Mgr* pParent /*=NULL*/)
	: CESL_Mgr(IDD, pParent)

{
	//{{AFX_DATA_INIT(CBO_CAT_REGNO_GIVE_LOST)
...
CBO_CAT_REGNO_GIVE_LOST::~CBO_CAT_REGNO_GIVE_LOST()
{
}

VOID CBO_CAT_REGNO_GIVE_LOST::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBO_CAT_REGNO_GIVE_LOST)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CBO_CAT_REGNO_GIVE_LOST, CDialog)
	//{{AFX_MSG_MAP(CBO_CAT_REGNO_GIVE_LOST)
...
/////////////////////////////////////////////////////////////////////////////
// CBO_CAT_REGNO_GIVE_LOST message handlers

BOOL CBO_CAT_REGNO_GIVE_LOST::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	// TODO: Add extra initialization here
	if (InitESL_Mgr(_T("SM_단행_정리_누락등록번호_부여")) < 0)
	{
...
//
	...
	CESL_DataMgr *pTmpDM = FindDM(_T("DM_단행_정리DM_책정보"));
	if ( NULL == pTmpDM ) 
	{
	...
	for (INT i = 1; i<=iRowCount; i++)
	{
		if (pGrid->GetAt(i-1,0) == _T("V"))	
			strRegCode = pTmpDM->GetCellData(_T("BB_등록구분_코드"), i-1);
	}
	SetControlData( _T("CM_단행_정리_누락등록번호_부여"), _T("등록구분"), strRegCode );
// 2008.05.22 ADD BY PDJ
	// 2008.05.22 ADD BY PDJ
	//  누락 입력의 등록구분도 동일하게 설정
	SetControlData( "CM_단행_정리_누락등록번호_부여", "입력_등록구분", strRegCode );

	GetDlgItem(IDC_EDT_COUNT)->SetWindowText(strbuf);

	// 원부번호 가져오기
	CESL_DataMgr *pDM = FindDM(_T("DM_단행_정리_누락등록번호_부여"));
	if (!pDM) return FALSE;

	CString strQuery;
	m_strYear = CTime::GetCurrentTime().Format(_T("%Y"));
		
	strQuery.Format(_T("SELECT MAX(ACCESSION_REC_NO) FROM CO_ACCESSION_REC_TBL WHERE ACCESSION_REC_MAKE_YEAR = %s AND MANAGE_CODE = UDF_MANAGE_CODE"), m_strYear);
	
	CString strAccNo;
	pDM->GetOneValueQuery(strQuery, strAccNo);

	SetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("원부번호"), strAccNo);
	( (CComboBox*) GetDlgItem (IDC_cREG_Change_CLASS) )->EnableWindow(FALSE);
	( (CComboBox*) GetDlgItem (IDC_cREG_CODE_DROP) )->EnableWindow(FALSE);
...
VOID CBO_CAT_REGNO_GIVE_LOST::Load_REGNO(CString mQry)
{
	CString strQry;
	INT ids;
	...
	//===================================================
	//2009.04.13 ADD BY PJW : 70000개 이상이 검색될경우 불가능 -> 10000개 까지 검색되도록 조건추가함.
	CString strOmitRegNoCnt;
	INT nOmitRegNoCnt;
	CString strRowNumQuery;
	CESL_ControlMgr * pCm = FindCM(_T("CM_단행_정리_누락등록번호"));
	pCm->GetControlMgrData(_T("누락번호갯수"), strOmitRegNoCnt);
	nOmitRegNoCnt = _ttoi(strOmitRegNoCnt)+1;
	strRowNumQuery.Format(_T("AND ROWNUM < %d "), nOmitRegNoCnt);
	strQry += strRowNumQuery;		
	//===================================================
	...
	CESL_Grid *pGrid = (CESL_Grid*)FindControl(_T("CM_단행_정리_누락등록번호_부여"), _T("REGNO_GRID"));
	pGrid->Display();	
}

BEGIN_EVENTSINK_MAP(CBO_CAT_REGNO_GIVE_LOST, CDialog)
    //{{AFX_EVENTSINK_MAP(CBO_CAT_REGNO_GIVE_LOST)
	ON_EVENT(CBO_CAT_REGNO_GIVE_LOST, IDC_GRID_REGNO, -600 /* Click */, OnClickGridRegno, VTS_NONE)
	//}}AFX_EVENTSINK_MAP
END_EVENTSINK_MAP()

VOID CBO_CAT_REGNO_GIVE_LOST::OnClickGridRegno() 
{
...
VOID CBO_CAT_REGNO_GIVE_LOST::OnBTNSearch() 
{
	CString sRegCode,sRegNO_s,sRegNO_e,strBuf,strQry;

	GetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("등록구분"), sRegCode);
	GetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("등록번호시작"), sRegNO_s);
	GetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("등록번호끝"), sRegNO_e);
...
VOID CBO_CAT_REGNO_GIVE_LOST::OnOK() 
{
	INT idx,Book_idx;
	SelectMakeList(_T("CM_단행_정리_누락등록번호_부여"), _T("REGNO_GRID"));
	idx = SelectGetHeadPosition(_T("CM_단행_정리_누락등록번호_부여"), _T("REGNO_GRID"));
	...
	CESL_DataMgr *pDM = FindDM(_T("DM_단행_정리_누락등록번호_부여"));
	if ( NULL == pDM ) {
		AfxMessageBox ( _T("DM_단행_정리_누락등록번호_부여") );
		return ;
	}

	CESL_DataMgr *pBookDM = FindDM(_T("DM_단행_정리DM_책정보"));
	if ( NULL == pDM ) {
		AfxMessageBox ( _T("DM_단행_정리_누락등록번호_부여") );
		return ;
	}
	...
	Book_idx = SelectGetHeadPosition(_T("CM_단행_정리_누락등록번호"), _T("MAIN_GRID"));

	CString strRegNo,strRegCode,strReg,strRecKey,strQry,ACC_KEY;
	// 원부 번호를 구한다. 
	GetControlData( _T("CM_단행_정리_누락등록번호_부여"), _T("원부번호"), ACC_KEY );
	ACC_KEY.TrimLeft();
	ACC_KEY.TrimRight();
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    //2003.10.09 이학중 수정 
    ...
    */
	...
	{
		strQry = _T("SELECT REC_KEY FROM CO_ACCESSION_REC_TBL WHERE ACCESSION_REC_NO =")+ACC_KEY+_T(" AND MANAGE_CODE = UDF_MANAGE_CODE AND ACCESSION_REC_MAKE_YEAR =")+m_strYear;	
...
// 	strReg.Format(_T("%s%010d"),strRegCode,_ttoi(strRegNo.GetBuffer(0)));
	...
	pBookDM->SetCellData(_T("BB_등록번호"),strReg,Book_idx);
	pBookDM->SetCellData(_T("BB_원부KEY"),ACC_KEY,Book_idx);
	...
	idx = SelectGetNext(_T("CM_단행_정리_누락등록번호_부여"), _T("REGNO_GRID"));
	Book_idx = SelectGetNext(_T("CM_단행_정리_누락등록번호"), _T("MAIN_GRID"));
...
VOID CBO_CAT_REGNO_GIVE_LOST::OnChangeGroup() 
{
	CBO_CAT_GROUP_INSERT Dlg(this);
	Dlg.DoModal();
...
VOID CBO_CAT_REGNO_GIVE_LOST::OnbINSERTDROPREGNO() 
{
	INT ids = -1;
	CString strRegCode;
	CString strRegNo;
	...
	GetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("입력_등록구분"), strRegCode);
	GetControlData(_T("CM_단행_정리_누락등록번호_부여"), _T("입력_등록번호"), strRegNo);
	...
	if (!pDM) return;
	// 마지막 번호보다 큰지 검사
	strQuery.Format(_T("SELECT LAST_NUMBER FROM CO_LAST_NUMBER_TBL WHERE KIND_CODE = 'REG_NO' AND DIVISION_VALUE = '%s' AND MANAGE_CODE = UDF_MANAGE_CODE")
				    ,strRegCode);
...
// 	strRegNoField.Format(_T("%s%010s"), strRegCode, strRegNo);
	...
	strQuery.Format(_T("SELECT COUNT(*) FROM BO_BOOK_TBL WHERE REG_NO = '%s' AND MANAGE_CODE = UDF_MANAGE_CODE"), strRegNoField);
	ids = pDM->GetOneValueQuery(strQuery, strResult);
	...
	strQuery.Format(_T("SELECT COUNT(*) FROM CO_DROP_ACCESSION_NO_TBL WHERE PUBLISH_FORM_CODE = 'MO' AND REG_CODE = '%s' AND REG_NO = %d AND MANAGE_CODE = UDF_MANAGE_CODE")
...
VOID CBO_CAT_REGNO_GIVE_LOST::OnSelchangecREGCODEDROP() 
{
	// TODO: Add your control notification handler code here
	...
	//기존의 등록번호
	strRegCode = pTmpDM->GetCellData(_T("BB_등록구분_코드"), 0);

	//멀티드롭 선택된 항목
	GetControlData( _T("CM_단행_정리_누락등록번호_부여"), _T("입력_등록구분"), strSelRegCode );
	

	if ( strSelRegCode != strRegCode )
	{
		AfxMessageBox(_T("등록구분을 확인해 주세요"));
		SetControlData( _T("CM_단행_정리_누락등록번호_부여"), _T("입력_등록구분"), strRegCode );
		return;
	}
	
}

VOID CBO_CAT_REGNO_GIVE_LOST::OnSelchangecREGChangeCLASS() 
{
	// TODO: Add your control notification handler code here
	CString strSelRegCode;
	CString strRegCode;
	...
	//멀티드롭 선택된 항목
	GetControlData( _T("CM_단행_정리_누락등록번호_부여"), _T("등록구분"), strSelRegCode );
...
HBRUSH CBO_CAT_REGNO_GIVE_LOST::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
