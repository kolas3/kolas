...
// 		sEndRegNo.Format( _T("%s%s"), sRegCode, sTmp );
		...
		//시작, 마지막등록번호
		CString sStartRegNo, sEndRegNo;
		sStartRegNo = m_pParent->m_pInfo->MakeRegNo(sRegCode,sNum10);
		...
		//마지막 등록번호
		sEndRegNo = m_pParent->m_pInfo->MakeRegNo(sRegCode,sTmp);
...
// 					sRegNo = sRegCode + sLastNum;
					...
					sLastNum.Format( _T("%d"), iLastNum );
					sRegNo = m_pParent->m_pInfo->MakeRegNo(sRegCode sLastNum);
