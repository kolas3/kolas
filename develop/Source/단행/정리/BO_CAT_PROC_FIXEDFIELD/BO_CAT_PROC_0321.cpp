...
CBO_CAT_PROC_0321::CBO_CAT_PROC_0321(CESL_Mgr* pParent, CString sDm, CMarc &m_marc /*=NULL*/)
...
CBO_CAT_PROC_0321::~CBO_CAT_PROC_0321()
{
}
...
//			m_strOutput += ( strErrMsg + strHelpMsg + _T("\r\n"));
	...
	if ( sItemData != _T("") ) {
		...
		SetControlData(_T("CM_BO_CAT_PROC_0321"), _T("발행년유형"), _T("s"));
	}
	...
	if ( sItemData != _T("") ) {
		...
		if ( iIntCh == 0 ) {
			...
			SetControlData(_T("CM_BO_CAT_PROC_0321"), _T("발행년1"), _T(""));
		}
	...
	*/
	...
	if ( sStatus != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("상태"), sStatus );
	}
	if ( sForm != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("레코드형태"), sForm );
	}
	if ( sSeoji != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("서지수준"), sSeoji );
	}
	if ( sInputLevel != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("입력수준"), sInputLevel );
	}
	if ( sMok != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("목록기술형식"), sMok );
	}
	if ( sRelation != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0321"), _T("연관레코드조건"), sRelation );
	}
...
EFS_BEGIN
	...
	*/
	...
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("레코드상태"), sStatus );
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("레코드형태"), sForm );
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("서지수준"), sSeoji );
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("입력수준"), sInputLevel );
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("목록기술형식"), sMok );
	GetControlData( _T("CM_BO_CAT_PROC_0321"), _T("연관레코드조건"), sRecord );
	...
	{	
		...
		//------------------------------------------
			...
			if (dlgLeader.DoModal() == IDOK) {			
				...
				SetControlData( _T("CM_BO_CAT_PROC_0321"), sControlAliasLeader, sCode );
		...
		CString sData;
		GetControlData( _T("CM_BO_CAT_PROC_0321"), sControlAliasLeader, sData );
		...
		if ( iLeaderYes == 1 ) {
			...
			if ( bFind == FALSE && sData != _T("") ) {
				...
				SetControlData( _T("CM_BO_CAT_PROC_0321"), sControlAliasLeader, _T("") );
				//return true;
		...
		sData = _T("");
		GetControlData( _T("CM_BO_CAT_PROC_0321"), sControlAlias, sData );
		...
		if ( bFind == FALSE && sData != _T("") && sValue != _T("(도법)") ) {
				AfxMessageBox ( _T("잘못된 값입니다.") );
				SetControlData( _T("CM_BO_CAT_PROC_0321"), sControlAlias, _T("") );
				//return true;
				return CDialog::PreTranslateMessage(pMsg);
		}
	}
...
HBRUSH CBO_CAT_PROC_0321::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
