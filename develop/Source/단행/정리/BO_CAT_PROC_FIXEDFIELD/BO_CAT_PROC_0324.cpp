...
CBO_CAT_PROC_0324::CBO_CAT_PROC_0324(CESL_Mgr* pParent, CString sDm, CMarc &m_marc /*=NULL*/)
...
CBO_CAT_PROC_0324::~CBO_CAT_PROC_0324()
{
}
...
EFS_BEGIN
	...
	if (InitESL_Mgr(_T("BO_CAT_PROC_0324")) < 0)
	{
...
//			m_strOutput += ( strErrMsg + strHelpMsg + _T("\r\n"));
	...
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("입력날짜"), sItemData);
	}
	else {
		//sItemData = _T("");
		GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("날짜") , sItemData );
		CString sDate;
		sDate.Format( _T("%s%s%s"), sItemData.Mid(2,2), sItemData.Mid(5,2), sItemData.Right(2) );
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("입력날짜") , sDate );
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@6"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("유형"), sItemData);
	}
	else {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("유형"), _T("s"));
	}
	...
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행년1"), sItemData);
	}
	*/
	INT iIntCh = 0;
	...
	if ( sItemData != _T("") ) {
		...
		if ( iIntCh == 0 ) {
			...
			SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행년1"), _T(""));
		}
	...
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행년2"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@15-17"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행국명"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@26-27"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("대학부호"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@28"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("수정레코드"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@32"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("목록전거"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@35-37"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("언어"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@38-39"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("정부기관"), sItemData);
	}


	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@18-20"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("상영시간"), sItemData);
	}	

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@22"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("이용대상수준"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@23"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료1"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@24"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료2"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@25"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료3"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@33"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("자료유형"), sItemData);
	}

	sItemData = _T("");
	m_pInfo->pMarcMgr->GetItem(&m_marc, _T("008@34"), sItemData);
	if ( sItemData != _T("") ) {
		SetControlData(_T("CM_BO_CAT_PROC_0324"), _T("기술수준"), sItemData);
	}
	...
	if ( sStatus != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("상태"), sStatus );
	}
	if ( sForm != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("레코드형태"), sForm );
	}
	if ( sSeoji != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("서지수준"), sSeoji );
	}
	if ( sInputLevel != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("입력수준"), sInputLevel );
	}
	if ( sMok != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("목록기술형식"), sMok );
	}
	if ( sRelation != _T(" ") ) {
		SetControlData( _T("CM_BO_CAT_PROC_0324"), _T("연관레코드조건"), sRelation );
	}
...
VOID CBO_CAT_PROC_0324::OnbVARIABLE() 
{
EFS_BEGIN
	...
	CString sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("입력날짜"), sData);
	...
	CString sYear1;
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행년1"), sYear1);
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행년2"), sData);
	...
	*/
	...
	sYear1 = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("유형"), sYear1);
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("발행국명"), sData);
	...
	*/
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("대학부호"), sData);
	...
	*/
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("수정레코드"), sData);
	...
	*/
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("목록전거"), sData);
	...
	*/
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("언어"), sData);
	...
	*/
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("정부기관"), sData);
	if ( sData.IsEmpty() ) {
		/*
		if ( sData == _T("") ) {
	...
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("상영시간"), sData);
	if ( sData == _T("") ) {
		/*
		if ( sData == _T("") ) {
			sMessage = sMessage + _T(", 상영시간");
		}
		*/
		sData = _T("   ");
	}
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@18-20"), sData );

	//21 빈칸

	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("이용대상수준"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@22"), sData );
		
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료1"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@23"), sData );

	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료2"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@24"), sData );

	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("딸린자료3"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@25"), sData );
	
	//29-31 빈칸

	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("자료유형"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@33"), sData );
	if ( sData == _T("") ) {
		sMessage = sMessage + _T(", 자료유형");
	}
	
	sData = _T("");
	GetControlData(_T("CM_BO_CAT_PROC_0324"), _T("기술수준"), sData);
	m_pInfo->pMarcMgr->SetItem(&m_marc, _T("008@34"), sData );
	if ( sData == _T("") ) {
		sMessage = sMessage + _T(", 기술수준");
	}
	...
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("상태"), sStatus );
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("레코드형태"), sForm );
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("서지수준"), sSeoji );
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("입력수준"), sInputLevel );
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("목록기술형식"), sMok );
	GetControlData( _T("CM_BO_CAT_PROC_0324"), _T("연관레코드조건"), sRecord );
...
BOOL CBO_CAT_PROC_0324::PreTranslateMessage(MSG* pMsg) 
{
EFS_BEGIN
	...
	{	
		...
		for ( INT i = 0 ; i < 4 ; i++ ) {
			...
			if ( pWnd->m_hWnd == pMsg->hwnd ) {
				...
				switch( i ) {
					case 0 : 
						iMode = 0;
						sControlAlias = _T("발행국명");
						break;
					case 1 : 
						iMode = 1;
						sControlAlias = _T("대학부호");
						break;
					case 2 :
						iMode = 2;
						sControlAlias = _T("언어");
				...
				if ( dlg.DoModal() == IDOK ) {
					...
					SetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAlias, sCode );
				}
				bScreenYn = TRUE;
			}
		}
		...
		if ( iLeaderYes == 1 ) {
			...
			if (dlgLeader.DoModal() == IDOK) {			
				...
				SetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAliasLeader, sCode );
		...
		//CWnd *pWnd;
		UINT wndCodeID[ 10 ] = {		
			IDC_cSTYLE, IDC_cUPDATE, IDC_cLIST_JUNGEO, 
			IDC_eRUNNING_TIME, IDC_cUSE_OBJ_LEVEL, IDC_cACCOMP_MAT1,
			IDC_cACCOMP_MAT2, IDC_cACCOMP_MAT3, IDC_cMETERIAL_TYPE,
			IDC_cTECH_LEVEL			
		};				
		...
		for ( i = 0 ; i < 10 ; i++ ) {
			...
			if ( pWnd->m_hWnd == pMsg->hwnd ) {
				...
				switch( i ) {
					...
					case 3 :
						sValue = _T("(상영시간)");
						sControlAlias = _T("상영시간");
						break;
					case 4 :
						sValue = _T("(이용대상자수준)");
						sControlAlias = _T("이용대상수준");
						break;
					case 5 :
						sValue = _T("(008딸린자료)");
						sControlAlias = _T("딸림자료1");
						break;
					case 6 :
						sValue = _T("(008딸린자료)");
						sControlAlias = _T("딸림자료2");
						break;
					case 7 :
						sValue = _T("(008딸린자료)");
						sControlAlias = _T("딸림자료3");
						break;
					case 8 :		
						sValue = _T("(시청각자료유형)");
						sControlAlias = _T("자료유형");
						break;
					case 9 :
						sValue = _T("(기술수준)");
						sControlAlias = _T("기술수준");
						break;					
				}			
			}
		}
		...
		CString sData;
		GetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAliasLeader, sData );
		...
		if ( iLeaderYes == 1 ) {
			...
			if ( bFind == FALSE && sData != _T("") ) {
				...
				SetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAliasLeader, _T("") );
				//return true;
		...
		sData = _T("");
		GetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAlias, sData );
		...
		if ( bFind == FALSE && sData != _T("") && sValue != _T("(도법)") ) {
				AfxMessageBox ( _T("잘못된 값입니다.") );
				SetControlData( _T("CM_BO_CAT_PROC_0324"), sControlAlias, _T("") );
				//return true;
				return CDialog::PreTranslateMessage(pMsg);
		}
	}
...
HBRUSH CBO_CAT_PROC_0324::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
