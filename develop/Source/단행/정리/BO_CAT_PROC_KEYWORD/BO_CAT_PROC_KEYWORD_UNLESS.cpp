...
EFS_BEGIN
	...
	CString sWord;
	GetControlData( _T("CM_BO_CAT_PROC_KEYWORD_UNLESS"), _T("불용어"), sWord );
	if ( sWord.IsEmpty() ){
...
EFS_BEGIN
	...
	/*
		CString strRecKey;	
		CString sQuery = _T("SELECT REC_KEY FROM CO_CHARSET_MANAGEMENT_TBL WHERE CHARSET_NAME = '역할어'");
	...
	*/
	...
	/*
		...
		for ( INT j = 0 ; j < iRowCount1 ; j++ ) {
			CString sRoleWord = pDM->GetCellData( _T("역할어"), j );
		}
	*/
