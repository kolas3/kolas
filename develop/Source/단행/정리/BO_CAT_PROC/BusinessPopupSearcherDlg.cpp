...
#include "..\..\..\관리\분류기호\MNG_CLASS_SIGN_MINI_POPUP\MNG_CLASS_SIGN_MINI_POPUP_01.h"
#include "..\..\..\관리\전거\ACMarcLittleGetter\ACMarcLittleGetter_01.h"
...
CString CBusinessPopupSearcherDlg::GetTitleQuery()
{
	...
	if( !tmp.GetLength() )
		...
		where.Format( _T(" CATSEARCH( I.TITLE, '%s', NULL ) > 0 AND"), tmp );
...
CString CBusinessPopupSearcherDlg::GetKeywordQuery()
{
	...
	for( INT i = 0; i < edtCnt; i++ )
	{
		...
		if( keyword[ i ].GetLength() )
			keyword[ i ] = _T(" CATSEARCH( I.KEYWORD, '") + keyword[ i ] + _T("', NULL ) > 0");
	}
...
CString CBusinessPopupSearcherDlg::GetAuthorQuery()
{
	...
	tmp.Replace( _T(" "), _T("") );
	if( tmp.GetLength() )
		query.Format( _T(" CATSEARCH( I.AUTHOR, '%s', NULL ) > 0 AND"), tmp );
...
CString CBusinessPopupSearcherDlg::GetPublisherQuery()
{
	...
	tmp.Replace( _T(" "), _T("") );
	if( tmp.GetLength() )
		query.Format( _T(" CATSEARCH( I.PUBLISHER, '%s', NULL ) > 0 AND"), tmp );
