...
BOOL CMISSING_DATA_SEARCH::OnInitDialog() 
{
	...
	if(InitESL_Mgr(_T("SM_SE_MISSING_SEARCH")) < 0)	{
...
VOID CMISSING_DATA_SEARCH::MakeQuery()
{
	...
	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("TITLE_FROM")	, sTitleFrom	);
	if( ids ) return;
	
	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("TITLE_TO")	, sTitleTo		);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("FREQ")		, sFreq	);
	if( ids ) return;
	
	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("CONTROLNO_FROM")	, sControlFrom		);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("ISSN1")		, sIssnFrom		);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("ISSN2")		, sIssnTo		);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("PROVIDER")	, sProviderCode	);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("CLAIM_SEQ1")	, sClaimSeq1	);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("CLAIM_SEQ2")	, sClaimSeq2	);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("PUB_DATE1")	, sPubDate1	);
	if( ids ) return;

	ids = GetControlData( _T("CM_SE_MISSING_SEARCH"), _T("PUB_DATE2")	, sPubDate2	);
	if( ids ) return;
...
if(ORA_CATSEARCH == m_pInfo->GetSearchIndexType())
	...
	// ===========================================================================================================
	// ISSN 2005.04.04 by SM5ong
	// ===========================================================================================================
	if( !sIssnFrom.IsEmpty() && !sIssnTo.IsEmpty()) {

		if( strWhere.IsEmpty() )
			strWhere = _T(" I.ST_ISSN = '") + sIssnFrom + _T("-") + sIssnTo + _T("' ");
		else
			strWhere += _T(" AND I.ST_ISSN = '") + sControlFrom + _T("-") + sIssnTo + _T("' ");
	}
	...
	if( strWhere != _T("") ) {
		...
		if( !sProviderCode.IsEmpty() ) {
			strOption.Format( _T(" AND M.PROVIDER_CODE = '%s' "), sProviderCode );
			strWhere += strOption;
		}
