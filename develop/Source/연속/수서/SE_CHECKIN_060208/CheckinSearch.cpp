...
CCheckinSearch::CCheckinSearch(CESL_Mgr* pParent /*=NULL*/)	: CESL_Mgr(IDD, pParent)
{
...
BOOL CCheckinSearch::OnInitDialog() 
{
	...
	if( str64 == _T("Y") )
	{
		...
		m_pCM->SetControlMgrData(_T("입수지연일"),strTime);
	}
	else
	{
		...
		GetDlgItem(IDC_edtCHE_ACQYEAR)->MoveWindow(311, 104, 56, 20);
		GetDlgItem(IDC_STATIC_ACQYEAR)->MoveWindow(254, 108, 56, 20);
		GetDlgItem(IDC_edtCHE_RECEIPTNO)->MoveWindow(421, 104, 56, 20);
		GetDlgItem(IDC_STATIC_RECEIPTNO)->MoveWindow(370, 108, 51, 20);
	}
	...
	else
	{
		...
		GetDlgItem(IDC_edtCHE_ACQYEAR)->MoveWindow(311, 128, 56, 20);
		GetDlgItem(IDC_STATIC_ACQYEAR)->MoveWindow(254, 132, 56, 20);
		GetDlgItem(IDC_edtCHE_RECEIPTNO)->MoveWindow(421, 128, 56, 20);
		GetDlgItem(IDC_STATIC_RECEIPTNO)->MoveWindow(370, 132, 51, 20);
	}
...
INT CCheckinSearch::MakeQuery()
{
	...
	CString ACQ_DELAY_DATE = _T("");			m_pCM->GetControlMgrData(_T("입수지연일"),ACQ_DELAY_DATE);
	...
	ACQ_DELAY_DATE = TrimAll(ACQ_DELAY_DATE);
	...
	if(!ACQ_DELAY_DATE.IsEmpty())
	{
	
		if(1 == m_nACQ_TYPE) 
		{
			strQuery += _T(" AND ");
			tmpquery.Format(_T("GET_EXP_START_DATE(P.ACQ_EXP_TYPE,P.ACQ_EXP_KEY) < '%s'"),ACQ_DELAY_DATE);
			strQuery += tmpquery;
		...
		{
			strQuery += _T(" AND ");
			tmpquery.Format(_T("GET_EXP_START_DATE(D.ACQ_EXP_TYPE,D.ACQ_EXP_KEY) < '%s'"),ACQ_DELAY_DATE);
			strQuery += tmpquery;
