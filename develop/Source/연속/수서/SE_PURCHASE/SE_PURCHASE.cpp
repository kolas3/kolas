...
__declspec(dllexport) CESL_Mgr* CreateModelessDlg(CString strSM, CString strButtonAlias, BOOL bReadOnly, CWnd *pWindow)
{
	...
	if(strButtonAlias == _T("주문처리"))
	{

		m_pWindow->OrderProcess();
	}

	if(strButtonAlias == _T("주문취소"))
	{

		m_pWindow->OrderCancel();
	}

	if(strButtonAlias == _T("파일반입"))
	{
	...
	//JOB.2019.0073
	/*
	if(strButtonAlias == _T("주문메일"))
	{

		m_pWindow->OrderMail();
	}
	*/
	if(strButtonAlias == _T("차수관리"))
	{

		m_pWindow->OrderSeqNo();
	}
	if(strButtonAlias == _T("구입가격일괄변경"))
	{
