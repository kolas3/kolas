...
#include "..\SE_API\SeMarcImportMarcView.h"
...
BOOL CSeMarcImport::OnInitDialog() 
{
	...
	if(InitESL_Mgr(_T("연속_수서_구입자료관리_자료반입"))<0) {
		AfxMessageBox( _T("InitESL_Mgr Error : 연속_수서_구입자료관리_자료반입") );
		return FALSE;
	}
	
	m_pCM = FindCM(_T("CM_연속_구입자료관리_자료반입"));
	m_pGrid = (CESL_Grid*)m_pCM->FindControl(_T("그리드"));
	m_pDM = FindDM(_T("DM_연속_구입자료관리_자료반입"));
	

	if(!m_pDM || !m_pCM || !m_pGrid) {
		AfxMessageBox( _T("DM,CM,grid Error : 연속_수서_구입자료관리_자료반입") );
		return FALSE;
	}	
...
VOID CSeMarcImport::OnbtnMARCIMPORTSEARCH() 
{
	...
	CString SUB_END_DATE = _T("");				m_pCM->GetControlMgrData(_T("구독만기일"),SUB_END_DATE);
	...
	SUB_END_DATE = 	TrimAll(SUB_END_DATE);
	...
	//소장종에 한하여 검색
	strQuery = _T("S.LOC_FLAG = '0' AND I.REC_KEY = S.REC_KEY AND I.REC_KEY = P.SPECIES_KEY");
	...
	//구독만기일
	if(!SUB_END_DATE.IsEmpty())
	{
		strQuery += _T(" AND ");
		//========================================================
		//20080606 UPDATE BY PJW : 구독만기일 검색일자 수정
		//tmpquery.Format(_T(" P.END_DATE < '%s'"), SUB_END_DATE);
		tmpquery.Format(_T(" P.END_DATE <= to_date('%s', 'YYYY/MM/DD')+0.99999"), SUB_END_DATE);
		//========================================================
		strQuery += tmpquery;
	}
