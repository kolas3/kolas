...
__declspec(dllexport) CWnd* CreateForm(CWnd *pParent, CESL_Information* pInfo, TCHAR VIEW_STYLE, CString strMenuAlias) 
{
	...
	if(_T("2120") == strMenuAlias) m_pWindow->m_nACQ_TYPE = 1; //구입 
	else if(_T("2210") == strMenuAlias) m_pWindow->m_nACQ_TYPE = 2; //기증 
...
__declspec(dllexport) CESL_Mgr* CreateModelessDlg(CString strSM, CString strButtonAlias, BOOL bReadOnly, CWnd *pWindow)
{
	...
	{
		m_pWindow->LaunchCheckinVol(TRUE);
	}

	if(strButtonAlias == _T("일괄입수처리"))
	{
		m_pWindow->LaunchCheckinVol(FALSE);
	}
	...
	if(strButtonAlias == _T("Sisac바코드 스캔"))
	{
		m_pWindow->SisacBarCode();
	}
