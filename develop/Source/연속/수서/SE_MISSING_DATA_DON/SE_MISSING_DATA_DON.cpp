// SE_MISSING_DATA_DON.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE SE_MISSING_DATA_DONDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("SE_MISSING_DATA_DON.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(SE_MISSING_DATA_DONDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(SE_MISSING_DATA_DONDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("SE_MISSING_DATA_DON.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(SE_MISSING_DATA_DONDLL);
	}
	return 1;   // ok
}
...
__declspec(dllexport) CESL_Mgr* CreateModelessDlg(CString strSM, CString strButtonAlias, BOOL bReadOnly, CWnd *pWindow)
{
	...
	if (strButtonAlias == _T("찾기")) {  
		...
		//m_pDlg->m_nACQ_TYPE = m_pWindow->m_nACQ_TYPE;
