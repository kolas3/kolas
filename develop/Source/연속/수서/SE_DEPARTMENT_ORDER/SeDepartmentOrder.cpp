...
CSeDepartmentOrder::~CSeDepartmentOrder()
{
}
...
BEGIN_MESSAGE_MAP(CSeDepartmentOrder, CDialog)
	...
	ON_BN_CLICKED(IDC_btnDepOrderPriority, OnbtnDepOrderPriority)
	ON_WM_CTLCOLOR()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
VOID CSeDepartmentOrder::RefreshAll()
{
	...
	if(m_strPURCHASE_SPECIES_KEY.IsEmpty())
		m_strPURCHASE_SPECIES_KEY = _T("NULL");

	if(m_strPURCHASE_MANAGE_KEY.IsEmpty())
		m_strPURCHASE_MANAGE_KEY = _T("NULL");
...
VOID CSeDepartmentOrder::SetAllData(INT row, CString strUDF)
{
	...
	if(m_nOpenLockMode>0 && m_nOpenMode<0)
	{

		m_pDM->SetCellData(_T("구입종KEY"),m_strPURCHASE_SPECIES_KEY,row);
...
VOID CSeDepartmentOrder::OnbtnDepOrderPriority() 
{
	// TODO: Add your control notification handler code here
	CSeDepartmentPriority dlg(this);
	dlg.DoModal();

}

VOID CSeDepartmentOrder::InsertDefaultAcqBookCnt(INT nDefaultCnt)
{
	m_pDM->InsertRow(-1);
	INT row = m_pDM->GetRowCount()-1;
	...
	data[0] = _T("");
	data[1].Format(_T("%d"),nDefaultCnt);
	m_pDM->SetCellData(alias,3,data,row);
	

	//UDF처리 
	m_pDM->SetCellData(_T("UDF_부서코드"),data[0],row);
	m_pDM->SetCellData(_T("UDF_상태"),_T("I"),row);
	m_pInfo->pCodeMgr->ConvertCodeToDescInDM(m_pDM,_T("자료실구분"),_T("UDF_부서코드"));

}
HBRUSH CSeDepartmentOrder::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
