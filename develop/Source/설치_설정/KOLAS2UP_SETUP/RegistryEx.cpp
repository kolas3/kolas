...
BOOL CRegistryEx::Write (LPCTSTR pszKey, LPCTSTR pszData)
{
	...
	//++2009.01.19 UPDATE BY CJY {{++
//DEL 유니코드일경우에는 바이트수가 문자열의 길이와 비일치
...
//DEL	이전소스
//DEL 	LONG ReturnValue = RegSetValueEx (m_hKey, pszKey, 0L, REG_SZ,
//DEL 		(CONST BYTE*) pszData, _tcsclen(pszData) + 1);
//DEL 	m_Info.lMessage = ReturnValue;
//DEL 	m_Info.dwSize = _tcsclen(pszData) + 1;
//DEL 	m_Info.dwType = REG_SZ;
	//--2009.01.19 UPDATE BY CJY --}}
...
BOOL CRegistryEx::Read (LPCTSTR pszKey, LPTSTR lpszBuffer, INT nBufferSize )
{
	ASSERT(m_hKey);
	ASSERT(pszKey);
