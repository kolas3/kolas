...
bool CZipper::ZipFolder(LPCTSTR szFilePath, bool bIgnoreFilePath, CString strStartDate, CString strEndDate)
{
	// make zip path
	char szDrive[_MAX_DRIVE], szFolder[MAX_PATH], szName[_MAX_FNAME];
	_splitpath(szFilePath, szDrive, szFolder, szName, NULL);
	...
	if (zip.OpenZip(szZipPath, FALSE))
	{
		zip.SetStratDate(strStartDate);
		zip.SetEndDate(strEndDate);
...
bool CZipper::AddFolderToZip(LPCTSTR szFolderPath, bool bIgnoreFilePath)
{
	...
	{
		//================================================
		//2008.10.10 UPDATE BY PJW : 압축 시작날짜,마지막날짜 체크
// 		do 
//		{
...
//		while (FindNextFile(hSearch, &finfo));
		...
		if( !m_strStartDate.IsEmpty() && !m_strEndDate.IsEmpty() )
		{
			bUseDate = TRUE;
		}
		else if( !m_strStartDate.IsEmpty() )
		{
			m_strEndDate = m_strStartDate;
			bUseDate = TRUE;
		}
		else if( !m_strEndDate.IsEmpty() )
		{
		...
		if( bUseDate )
		{
			do 
			{
				if (finfo.cFileName[0] != '.') 
				{
					strFileDate = finfo.cFileName;

					strFileDate.Replace(_T("("),_T(""));
					strFileDate.Replace(_T("색인종책값"),_T(""));
					strFileDate.Replace(_T("search_"),_T(""));
					strFileDate.Replace(_T("ESL_DM_"),_T(""));
					strFileDate.Replace(_T("사본 - "),_T(""));

					nFileDate = atoi(strFileDate.Left(8));
					nStartDate = atoi(m_strStartDate);
					nEndDate = atoi(m_strEndDate);
					if( (nStartDate <= nFileDate && nEndDate >= nFileDate)  ||  strFileDate == _T("FILELOG") )
					{
						char szItem[MAX_PATH];
						_makepath(szItem, szDrive, szFolder, finfo.cFileName, NULL);
...
//================================================
//2008.10.10 ADD BY PJW : 압축 시작날짜,마지막날짜 저장
bool CZipper::SetEndDate(CString strEndDate)
{
//	CZipper zip;
//	zip.m_strEndDate = strEndDate;
	m_strEndDate = strEndDate;

	return TRUE;
}

bool CZipper::SetStratDate(CString strStartDate)
{
