...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd, CDialog *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
    ...
    m_dwStyle = CBRS_ALIGN_TOP | dwStyle & CBRS_ALL ;

	// create the base window
    CString wndclass = AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW),
        m_brushBkgd, 0);
    if (!CWnd::Create(wndclass, pTitle, dwStyle | WS_BORDER, CRect(0,0,0,0),
        pParentWnd, 0))
        return FALSE;
	...
	m_sizeHorz.cy += m_cxEdge;
	m_sizeVert.cx += m_cxEdge;
