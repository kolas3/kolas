...
void BCMenuData::SetAnsiString(LPCTSTR szAnsiString)
{
	USES_CONVERSION;
	SetWideString(szAnsiString);  //SK:  see MFC Tech Note 059
}
...
BCMenuData::~BCMenuData()
{
	...
	delete[] m_szMenuText; //Need not check for NULL because ANSI X3J16 allows _T("delete NULL")
}


void BCMenuData::SetWideString(LPCTSTR szWideString)
{
	delete[] m_szMenuText;//Need not check for NULL because ANSI X3J16 allows _T("delete NULL")
...
BOOL BCMenu::AppendODMenuA(LPTSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
	return AppendODMenuW(lpstrText,nFlags,nID,nIconNormal);//SK: See MFC Tech Note 059
}
...
{
	USES_CONVERSION;
	return AppendODMenuW(lpstrText,nFlags,nID,il,xoffset);
}
...
BOOL BCMenu::InsertODMenuA(UINT nPosition,LPTSTR lpstrText,UINT nFlags,UINT nID,
                           int nIconNormal)
{
	USES_CONVERSION;
	return InsertODMenuW(nPosition,lpstrText,nFlags,nID,nIconNormal);
}
...
{
	USES_CONVERSION;
	return InsertODMenuW(nPosition,lpstrText,nFlags,nID,il,xoffset);
}
...
//--------------------------------------------------------------------------
//[21.06.99 rj]
BCMenu* BCMenu::AppendODPopupMenuA(LPTSTR lpstrText)
{
	USES_CONVERSION;
	return AppendODPopupMenuW(lpstrText);
}
