...
BEGIN_MESSAGE_MAP(CECO_HtmlViewer, CHtmlView)
	...
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
	ON_MESSAGE(WM_USER + 2000  , OnExecuteItem)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CECO_HtmlViewer::OnStatusTextChange(LPCTSTR lpszText) 
{
	...
	strText.MakeLower();
	if(strText.Find(_T("event:")) == -1 && strText.Find(_T("move:")) == -1)
	{
...
VOID CECO_HtmlViewer::OnBeforeNavigate2(LPCTSTR lpszURL, DWORD nFlags, LPCTSTR lpszTargetFrameName, CByteArray& baPostedData, LPCTSTR lpszHeaders, BOOL* pbCancel)
{
	...
	if(strText.Find(_T("event:")) >= 0)
	{
		...
		if ( m_pParent->GetSafeHwnd() != NULL ) 
			//m_pParent->SendMessage(WM_USER + 90, NULL, (LPARAM)strText.GetBuffer(0));	
		//m_pParent->SendMessage(WM_USER + 87 , NULL, (LPARAM)(LPCTSTR)strText);
		SendMessage(WM_USER +2000 , NULL, (LPARAM)(LPCTSTR)strText);
		*pbCancel = TRUE;
		//return;
	}
	else if(strText.Find(_T("move:")) >= 0)
	{
		strText.Replace(_T("move:"), _T(""));
		if ( m_pParent->GetSafeHwnd() != NULL ) 
			m_pParent->SendMessage(WM_USER + 89, NULL, (LPARAM)strText.GetBuffer(0));	
		*pbCancel = TRUE;
		return;
...
// 18.11.07 JMJ KOL.RED.2018.020
VOID CECO_HtmlViewer::OnExecuteItem(WPARAM wParam,LPARAM lParam)
{	
	if ( m_pParent->GetSafeHwnd() != NULL ) 
		m_pParent->SendMessage(WM_USER + 87 , NULL, lParam);
}

// 18.12.27 JMJ KOL.RED.2018.020.001 추가보완
VOID CECO_HtmlViewer::SetOption(CString strParentID, CString strEvent, CString strDesc)
{
...
if (pDoc2 != NULL)
{
	...
	if (SUCCEEDED(hr))
	{
		OLECHAR FAR* szFunction = _T("AddKolasMenu");
		DISPID dispid;
		...
		if (SUCCEEDED(hr))
		{
			...
			// 파라미터 3개라서 VTS_BSTR 3번
			static BYTE parms[] = VTS_BSTR VTS_BSTR VTS_BSTR;
			COleDispatchDriver dispDriver(pDisp, FALSE);

			// 자바스크립트 파라미터 3개 던짐 parms -> 배열갯수
			dispDriver.InvokeHelper(dispid, DISPATCH_METHOD, VT_VARIANT,
				(void*)&vtResult, parms, (LPCTSTR)strParentID, (LPCTSTR)strEvent, (LPCTSTR)strDesc);
		}
	}
	pDoc2->Release();
}
