...
BOOL CHTMLListCtrl::Create(CWnd *pParent, CRect rc, UINT nID,DWORD dwStyle)
{
	...
	if(!CWnd::Create(sWindowClassName,_T("HTMLListCtrl"),dwStyle,rc,pParent,nID))
	{
		return FALSE;
	}
...
BOOL CHTMLListCtrl::DestroyWindow()
{
	DeleteAllItems();
