...
// 127 202 254
#define TOOLTIP_BK_COLOR RGB(254, 202, 127)//RGB(254, 202, 127)//RGB(160, 200, 100)
/*

* Method preamble ************************************************************
*
...
void TFXDataTip::Initialise( )
{
    if (_font.GetSafeHandle( ) == NULL)
    {
        ...
        LOGFONT lFont;
	//++2008.07.17 UPDATE BY CJY {{++
	// 한글폰트 문제로 인해 변경
	GetObject(GetStockObject(DEFAULT_GUI_FONT), sizeof lFont, &lFont);
        //DEL GetObject(GetStockObject(ANSI_VAR_FONT), sizeof lFont, &lFont);
	//--2008.07.17 UPDATE BY CJY --}}
        _font.CreateFontIndirect(&lFont);
	

        // create the other GDI objects
	_brush = new CBrush(TOOLTIP_BK_COLOR);
	
      //DEL   _brush = new CBrush(::GetSysColor(COLOR_INFOBK));
    }
...
//DEL     CSize size = dc.GetTextExtent(m_tip);
    
    CRect rSize = CalculateTextSize(m_tip, CRect(0, 0, 1, 1), &_font);

    // capture the mouse in order to be able to 
    // determine when to hide the window
 
//DEL	SetCaputre때문에 왼쪽버튼을 한번 더 눌러야 하는 현상 발생
//DEL     SetCapture();

    // determine the window size and position
    // displayed above and centered on the origin
//DEL     CRect wndRect(m_origin.x, 
//DEL                   m_origin.y, 
//DEL                   m_origin.x + size.cx, 
//DEL                   m_origin.y + size.cy); 
    CRect wndRect(
	    m_origin.x, 
	    m_origin.y, 
	    m_origin.x + rSize.Width(), 
	    m_origin.y + rSize.Height()
	    ); 
	...
	////////////////////////////////////////////////////////////////////
	// 여기부터 툴팁의 위치가 화면밖으로 벗어나는 것 방지
	LONG cxScreen = GetSystemMetrics(SM_CXSCREEN);
	LONG cyScreen = GetSystemMetrics(SM_CYSCREEN);

	if(wndRect.right >= cxScreen)	
	{
		INT xOffer = wndRect.Width() + m_offset.x;
		if(0 > wndRect.left-xOffer)
		{
			xOffer = wndRect.right - cxScreen;
		}
		wndRect.OffsetRect(CPoint(-xOffer, 0));
	}
	
	if(wndRect.bottom >= cyScreen)
	{
		INT yOffer = wndRect.Height() + m_offset.y;
		if(0 > wndRect.top-yOffer)
		{
			yOffer = wndRect.bottom - cyScreen;
		}
		wndRect.OffsetRect(CPoint(0, -yOffer));
	}
	// 여기까지 툴팁의 위치가 화면밖으로 벗어나는 것 방지
	////////////////////////////////////////////////////////////////
...
//DEL 	dc.SetBkColor(::GetSysColor(COLOR_INFOBK));
	dc.SetBkColor(TOOLTIP_BK_COLOR);
	dc.SetTextColor(::GetSysColor(COLOR_INFOTEXT));

	// draw the data tip

	CRect rText = CRect(2, 2, rect.Width(), rect.Height());
	dc.DrawText(m_tip, &rText, DT_EDITCONTROL);	
...
CRect TFXDataTip::CalculateTextSize(const CString strText, const CRect rTextRect, CFont* pFont)
{	
...
// 함수이름: TFXDataTip::Display
// 함수설명: 특정위치에 출력
// 반환  형: void 
// 인자[CPoint Mouse_Pos] : 
// 인자[CPoint Draw_Pos] : 
// 인자[const CString &tip] : 
void TFXDataTip::Display(CPoint Mouse_Pos, CPoint Draw_Pos, const CString &tip)
{
    if (!m_ready) return;
...
//DEL                   m_origin.y + size.cy); 
    CRect wndRect(
	    Draw_Pos.x-m_offset.x, 
	    Draw_Pos.y-m_offset.y, 
	    Draw_Pos.x-m_offset.x + rSize.Width(), 
	    Draw_Pos.y-m_offset.y + rSize.Height()
	    ); 
    ...
    // update window position and display
     ...
     if(TRUE == bRedraw) Invalidate();
    // define the active area around the cursor position
//DEL     m_captureRect = CRect(m_origin.x - _horizontal, 
//DEL                           m_origin.y - _vertical,
//DEL                           m_origin.x + _horizontal,
//DEL                           m_origin.y + _vertical);
	m_captureRect = CRect(0, 0, 1024, 1024);
    // set the current data tip window
    _current = this;
}
