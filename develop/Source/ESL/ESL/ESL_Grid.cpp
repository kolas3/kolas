...
UINT GridDisplayThread(LPVOID pParam)
{
	EFS_BEGIN
		TRACE (_T("(Start) - Grid display Therad \n"));
	DWORD START, FINISH;
	START = GetTickCount();
	CESL_Grid *pGrid = (CESL_Grid*)pParam;
	...
	if (nRows == 0) ((CESL_Grid*)pData->pGrid)->SetRows(2);
	else ((CESL_Grid*)pData->pGrid)->SetRows(nRows+1);
	...
	for (i = 0; i < nRows; i++) {
		...
		((CESL_Grid*)pData->pGrid)->SetTextMatrix(i+1, 0, strData);
		for (j = 0; j < nColumns; j++) {
			...
			if( parrColumnInfo[j]->bPasswordStyle )
				((CESL_Grid*)pData->pGrid)->SetTextMatrix(i+1, j+1, _T("***"));
			else
				((CESL_Grid*)pData->pGrid)->SetTextMatrix(i+1, j+1, strData);
		}		
	}	
	
	pData->m_Status = THREAD_NOTRUN;
	FINISH = GetTickCount();
	TRACE(_T("(Finish) - Grid display Therad [%d.%d sec]\n"), (FINISH-START)/1000, (FINISH-START)%1000);
...
CESL_Grid::CESL_Grid(CESL_Control_Element * ELEMENT)
{
...
INT CESL_Grid::GridEdit(INT row, INT col)
{
	EFS_BEGIN
...
VOID CESL_Grid::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	EFS_BEGIN	
	...
	if (GetMouseRow() == 0) {
		...
		if (m_bGRIDSORT == TRUE) {
			...
			m_strSortAlias = pColumnInfo->strColumnDMFieldAlias;
			m_bIsDesc = pColumnInfo->IsDesc;
...
VOID CESL_Grid::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if ( m_bCheck && m_cSelectType != 'N' && m_nSelectIdx == col-1) {
		if (GetMouseCol() != 0) {
			...
			if ( check ) SetCheck(GetRow()-1, false);
			else SetCheck(GetRow()-1, true);
		}
	}
...
VOID CESL_Grid::OnGridSelEditor(WPARAM w, LPARAM l)
{
	EFS_BEGIN
...
BOOL CESL_Grid::PreTranslateMessage(MSG* pMsg) 
{
	EFS_BEGIN
		if ( pMsg->message == WM_KEYDOWN ) {
			if ( pMsg->wParam == VK_RETURN ) {
				...
				if(ESL_GRID_NOT_USE_F12 != (m_uUseKeyFlag & ESL_GRID_NOT_USE_F12) )
				{
					...
					AfxMessageBox(_T("해당 메뉴의 출력권한이 없습니다."));
				}
				return true;
		...
		else if (pMsg->message == WM_COMMAND && LOWORD(pMsg->wParam) == 4005)
		{
			if (GetParent() && GetParent()->PreTranslateMessage(pMsg))
				return TRUE;
		}
...
VOID CESL_Grid::OnMouseMove(UINT nFlags, CPoint point) 
{
	EFS_BEGIN
		if ( m_point != point ) {
			CMSHFlexGrid::OnMouseMove(nFlags,point);
			DisplayToolTip();
			m_point = point;
		}
	EFS_END
...
VOID CESL_Grid::MainToolBarProc(UINT nIDC)
{
	EFS_BEGIN
...
INT CESL_Grid::FindLinkDMAndAlias(INT nCol, CString &strDMAlias, CString &strDMFieldAlias)
{
	EFS_BEGIN
...
INT CESL_Grid::InitColumnLinkDMAndAlias(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::InitColumnInfo(VOID)
{
	EFS_BEGIN
	...
	for (INT i = 0; i < nColumns; i++) {
		...
		BOOL bEncYN;
		if (arrEncryptColList.Lookup(pColumnInfo->strColumnDMFieldAlias,bEncYN ))
		{
			pColumnInfo->bPasswordStyle = TRUE;
		}	
	}
	return 0;
...
VOID CESL_Grid::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
INT CESL_Grid::Display(INT index)
{
	EFS_BEGIN
	...
	::AfxBeginThread(GridDisplayThread, this);
...
INT CESL_Grid::FreeWorkDlg(VOID)
{
	if (m_pWorkDlg) {
		delete m_pWorkDlg;
		m_pWorkDlg = NULL;
...
INT CESL_Grid::CountLinkColumns(VOID)
{	
	EFS_BEGIN
...
INT CESL_Grid::ParseMgrAndColumn(CString link_data, CString &MgrAlias, CString &ColumnAlias)
{
	EFS_BEGIN
...
INT CESL_Grid::UnSelectAll(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::SetColumnInfo(VOID)
{
	EFS_BEGIN
...
VOID CESL_Grid::OnTimer(UINT nIDEvent) 
{
	EFS_BEGIN
		if (nIDEvent == 0) {
			CESL_GridData *pData = (CESL_GridData *)m_pData;
		...
		CMSHFlexGrid::OnTimer(nIDEvent);
		
		EFS_END
}

void CESL_Grid::InitToolTip()
{
 	if(NULL != hTip)
 	{
 		return;
...
CRect CESL_Grid::CalculateTextSize(const CString strText, const CRect rTextRect)
{ 			
	CDC* pDC = GetDC();				
...
bool CESL_Grid::UseUserDefine(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::UserSetColumnInfo(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::OnGridConfig(VOID)
{
	EFS_BEGIN
...
VOID CESL_Grid::OnExcelExport(VOID)
{
	EFS_BEGIN
	...
	CString strConvertColFile;
	CString strLine;
	CStringArray astrConvertColList;
	
	
	astrConvertColList.RemoveAll();
	strConvertColFile = _T("..\\CFG\\Grid\\ConvertCol.cfg");
	CFileFind ff;
	if ( ff.FindFile(strConvertColFile) )
	{
		CStdioFile fd;
		if (fd.Open(strConvertColFile, CFile::modeRead|CFile::typeBinary) == TRUE) 		
		{
			TCHAR cUni;
			fd.Read( &cUni, sizeof(TCHAR) );
			if( 0xFEFF != cUni )
			...
			while(TRUE) 
			{
				if (fd.ReadString(strLine) == FALSE)
				{
					break;
				}
				strLine.TrimLeft();
				...
				{
					astrConvertColList.Add(strLine);
				}
			}
		}	
	}
	...
	if(NULL != ELEMENT)
	{
		...
		{ 
			...
			{	
				...
				if( pMgr->PrivacyPrintLogRecord(pDM,m_strPrintInfo, printType , 0) < 0 ) return;
			}
		}
	}
	for( i=0; i<nRow; i++ )
	{
		...
		{
			CString strColAlias;
			CString strConvertCol;
			INT bConvertCol = FALSE;
			strColAlias = GetTextMatrix(0, j);
			for( INT nConvertColIdx = 0 ; nConvertColIdx < astrConvertColList.GetSize() ; nConvertColIdx++)
			{
				strConvertCol = astrConvertColList.GetAt(nConvertColIdx);
				if( 0 <= strColAlias.Find(strConvertCol) )
				{
					bConvertCol = TRUE;
					break;
				}
			...
			if ( GetColWidth(j, 0) > 0 )
 			{
				...
				if ( i>0 && !strData.IsEmpty() && ( strData.Left(1) >= _T("0") && strData.Left(1) <= _T("9")) && TRUE == bConvertCol )
				{
					strTmpData = strTmpData + _T("=\"") + strData + _T("\"") + _T("\t");
				}
				else
...
INT CESL_Grid::OnExcelSetColumnInfo(VOID)	// Init_value를 바탕으로 Column info를 셋팅
{
	EFS_BEGIN
...
INT CESL_Grid::AddPopupMenuItem(CView *pView, TCHAR VIEW_STYLE, CArray <CString, CString> *pRemoveListArray/*=NULL*/)
{
	EFS_BEGIN
...
BOOL CESL_Grid::FindInPopupMenuRemoveList(CString &strRemoveText, CArray <CString, CString> *pRemoveListArray)
{
	EFS_BEGIN
...
INT CESL_Grid::InitPopupMenuItem(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::FreePopupMenuItem(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::FreeColumnInfo(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::InitGridConfig(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::ReadConfigFile(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::SetData(CString strHeader, CString strData)
{
	EFS_BEGIN
...
INT CESL_Grid::DisplayLine(INT nRowIdx)
{
	EFS_BEGIN
...
INT CESL_Grid::SetSort(BOOL bFlag)
{
	EFS_BEGIN
...
INT CESL_Grid::RefreshSelectedRows(VOID)
{
	EFS_BEGIN
...
INT CESL_Grid::ReshowSelectedRows(VOID)
{
	EFS_BEGIN
...
VOID CESL_Grid::SetProtectSelectColumn(BOOL bFlag)
{
	EFS_BEGIN
...
INT CESL_Grid::SELEDITOR_AddColumn(UINT nColumn)
{
	m_arrSelEditor.Add(nColumn);
	return 0;
}

INT CESL_Grid::SELEDITOR_RemoveColumn(UINT nColumn)
{
...
INT CESL_Grid::SELEDITOR_FindColumn(UINT nColumn)
{
...
INT CESL_Grid::SELEDITOR_RemoveAllColumns(VOID)
{
	m_arrSelEditor.RemoveAll();
	return 0;
}

INT CESL_Grid::SELEDITOR_GetColumnCount(VOID)
{
	return m_arrSelEditor.GetSize();
}

INT CESL_Grid::ReadExcelExportConfigFile(short &title_size, short &title_bk, short &title_fk, bool &title_bold, bool &title_italic, short &data_size, short &data_bk, short &data_fk, bool &data_bold, bool &data_italic)
{
	CStdioFile fd;
...
INT CESL_Grid::SetColumnEncryption( CString strAlias, INT bEncrptionType)
{
	EFS_BEGIN	
		
		arrEncryptColList.SetAt(strAlias, bEncrptionType);	
