...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    for (i = 0; i < m_nCols; i++)
	{
		...
		m_arColFormat[i] = m_cellDefault.GetFormat();
	}
    ...
    TRY 
    {
        ...
        if (addedCols > 0)
        {
            ...
            for (int col = startCol; col < nCols; col++)
			{
				...
				m_arColFormat[col] = m_cellFixedColDef.GetFormat();
			}
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    TRY
    {
        ...
        else
        {
			...
			m_arColFormat.InsertAt(nColumn, (UINT)0);
            if (!GetVirtualMode())
            {
                for (int row = 0; row < m_nRows; row++) 
                {
	...
	m_arColFormat[nColumn] = m_cellFixedColDef.GetFormat();
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
    ...
    pCell->SetFormat(pCell->GetDefaultCell()->GetFormat() | m_arColFormat[nCol]);
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
	...
	m_arColFormat.RemoveAt(nColumn);
    m_nCols--;
