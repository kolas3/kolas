...
//
//  U Need 2 Add _T("CPicture.CPP") and _T("CPicture.H") Into Your Project (From FileView)
...
//  #include _T("Picture.h") // Make Sure U Include This Where U Gonna Create The Object...
//
//  Load Picture Data Into The IPicture Interface (.BMP .DIB .EMF .GIF .ICO .JPG .WMF)
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//	m_Picture.Load(_T("Test.JPG")); // Load From a File - Just Load It (Show Later)
//	m_Picture.Load(IDR_TEST, _T("JPG")); // Load From a Resource - Just Load It (Show Later)
//  (U Must Include IDR_TEST In Your Resources Under a Custom Name, 4 Example - _T("JPG"))
//  
...
//
//  OR When Using a Pointer On a _T("Regular") MFC Application (CDC* pDC)
//  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
//
// Note:   When Adding a Bitmap Resource It Would Automatically Show On _T("Bitmap")
// ~~~~    This NOT Good Coz We Need To Load It From a Custom Resource _T("BMP")
//         To Add a Custom Rresource: Import Resource -> Open As -> Custom
//         (Both .BMP And .DIB Should Be Found Under _T("BMP"))
//
// InPut:  ResourceName - As a UINT Defined (Example: IDR_PICTURE_RESOURCE)
// ~~~~~   ResourceType - Type Name (Example: _T("JPG"))
//
...
//=============================================================================
{
	...
	if(hSource == NULL)
		...
		// WSO MessageBoxEx(hWnd, _T("FindResource() Failed\t"), ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
		return(FALSE);
		}
	...
	if(hGlobal == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		// WSO MessageBoxEx(hWnd, _T("LoadResource() Failed\t"), ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
		return(FALSE);
		}
	...
	if(lpVoid == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		// WSO MessageBoxEx(hWnd, _T("LockResource() Failed\t"), ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
		return(FALSE);
		}
	...
	if(m_IPicture != NULL) // Do Not Try To Read From Memory That Is Not Exist...
	    ...
	    // Calculate Its Size On a _T("Standard") (96 DPI) Device Context
...
//=============================================================================
{
	...
	if(PictureFile.Open(sFilePathName, CFile::modeRead | CFile::typeBinary, &e))
		{
	TCHAR cUni;
	PictureFile.Read( &cUni, sizeof(TCHAR) );
	if( 0xFEFF != cUni )
	{
		PictureFile.SeekToBegin();
	}
		...
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		// WSO MessageBoxEx(hWnd, szCause, ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
		bResult = FALSE;
		}
...
//=============================================================================
{
	...
	if(hGlobal == NULL)
		{
		HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
		// WSO MessageBoxEx(hWnd, _T("Can not allocate enough memory\t"), ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
		return(FALSE);
		}

	VOID* pData = GlobalLock(hGlobal);
	wmemcpy(pData, pBuffer, nSize);
	...
	if(CreateStreamOnHGlobal(hGlobal, TRUE, &pStream) == S_OK)
		...
		if((hr = OleLoadPicture(pStream, nSize, FALSE, IID_IPicture, (LPVOID *)&m_IPicture)) == E_NOINTERFACE)
			{
			HWND hWnd = AfxGetApp()->GetMainWnd()->m_hWnd;
			// WSO MessageBoxEx(hWnd, _T("IPicture interface is not supported\t"), ERROR_TITLE, MB_OK | MB_ICONSTOP, LANG_ENGLISH);
			return(FALSE);
			}
		else // S_OK
			{
...
//=============================================================================
{
	...
	// Cut UnNeeded Data Coming From SaveAsFile() (Leave Only _T("Pure") Picture Data)
	...
	// Allocate Only The _T("Pure") Picture Data
	RealData.LowPart = 0;
	RealData.HighPart = 0;
	...
	if(BitmapFile.Open(sFilePathName, CFile::typeBinary | CFile::modeCreate | CFile::modeWrite, &e))
		...
		if( 0 == BitmapFile.GetLength() )
		{			
			TCHAR cUni = 0xFEFF;
			BitmapFile.Write( &cUni, sizeof(TCHAR));
		}
...
//=============================================================================
{
	...
	// Use a _T("Standard") Print (When Printing)
    if(pDC->IsPrinting())
		{
		CurrentDPI_X = 96;
        CurrentDPI_Y = 96;
