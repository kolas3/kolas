...
SHORT col;
_TINT c;
for ( col = 0; col < 2047 && (c=FdkFgetc(fp)) != _TEOF && c != _T('\n'); col++) s_line[col] = c;
...
switch (option) {
case 1: // same character location search.
for(i = 1; i <= range; i++) if (ch == ctmp[i-1]) return i;
case 2: // different character location search.
for(i = 1; i <= range; i++) if (ch != ctmp[i-1]) return i;
}
...
TokenStr = _tcstok( NULL, _T("^") ); // BYEONGGEOL SUJEONG _T("=") -> _T("^")EURO.. 2002NYEON 1WEOL 18IL GEUMYOIL
if ( NULL == TokenStr ) {
s_data[0] = _T('\0');
} else {
...
OldDataPtr[(*tokenCnt)-1] = (_TCHAR *)malloc( i - OldPtr +sizeof(TCHAR));
...
OldDataPtr[(*tokenCnt)-1] = (_TCHAR *)malloc( length - OldPtr +sizeof(TCHAR));
if ( OldDataPtr[(*tokenCnt)-1] == NULL ) {
for ( i=0; i < *tokenCnt-1; i++ ) free( OldDataPtr[i] );
free( OldDataPtr );
return -5;
