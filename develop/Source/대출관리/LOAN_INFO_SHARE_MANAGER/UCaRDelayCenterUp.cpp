// UCaRDelayCenterUp.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CUCaRDelayCenterUp dialog


CUCaRDelayCenterUp::CUCaRDelayCenterUp( CESL_Mgr* pParent )
	:CShareInfoCenterUp(pParent)
{
	//{{AFX_DATA_INIT(CUCaRDelayCenterUp)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	m_pUClassInfoMapping = new CTableFieldMapping(this,UCLASS_REG_DELAY_ENV);
}

CUCaRDelayCenterUp::~CUCaRDelayCenterUp()
{
	delete m_pUClassInfoMapping;
}

VOID CUCaRDelayCenterUp::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUCaRDelayCenterUp)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CUCaRDelayCenterUp, CDialog)
	//{{AFX_MSG_MAP(CUCaRDelayCenterUp)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CUCaRDelayCenterUp message handlers



INT CUCaRDelayCenterUp::SetData(UPLOAD_SHARE_INFO UploadInfo , CString sLibCode ) 
{
	EFS_BEGIN
...
INT CUCaRDelayCenterUp::UploadData()
{
	EFS_BEGIN
...
INT CUCaRDelayCenterUp::InsertEnv()
{
	EFS_BEGIN
...
INT CUCaRDelayCenterUp::UpdateEnv()
{
	EFS_BEGIN
...
INT CUCaRDelayCenterUp::DeleteEnv()
{
	EFS_BEGIN
...
/// CLINET 에서 정보 구해오기
INT CUCaRDelayCenterUp::MakeLoaclEnvInfo()
{
	EFS_BEGIN
	...
	// 로컬 정보
	CESL_DataMgr* pDM = FindDM(_T("DM_CLIENT_UCAR_DELAY_INFO"));
...
/// 직급별 환경 정보 INSERT
INT CUCaRDelayCenterUp::InsertLocalEnvToCenter()
{
	EFS_BEGIN
...
/// 직급별 환경 정보 UPDATE
INT CUCaRDelayCenterUp::UpdateLocalEnvToCenter()
{
	EFS_BEGIN
...
/// [4102] 직급별 환경 정보 삭제
INT CUCaRDelayCenterUp::DeleteCenterEnvInfo( CString sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("DELETE FROM CT_POSREG_LOAN_ENV_TBL WHERE REC_KEY = %s ;")
		, sCenterEnvKey );
...
INT CUCaRDelayCenterUp::IsCenterLoanEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("SELECT REC_KEY FROM CT_POSREG_LOAN_ENV_TBL WHERE LIB_INFO_KEY = %s AND CLIENT_REC_KEY = %s ")
...
// [4103] INSERT
INT CUCaRDelayCenterUp::UploadUClassDelayEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN

	INT ids;
	
	CString sTableName = _T("CT_POSREG_LOAN_ENV_TBL");
	
	CESL_DataMgr* pLocalLoanDM = FindDM( _T("DM_CLIENT_UCAR_DELAY_INFO") );
...
// [4203] UPDATE
INT CUCaRDelayCenterUp::UpdateUClassDelayEnv( CString sCenterEnvKey )
{
	EFS_BEGIN
