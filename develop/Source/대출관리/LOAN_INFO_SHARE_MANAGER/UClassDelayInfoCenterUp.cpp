// UClassDelayInfoCenterUp.cpp : implementation file
//

#include "stdafx.h"
#include "UClassDelayInfoCenterUp.h"
...
/////////////////////////////////////////////////////////////////////////////
// CUClassDelayInfoCenterUp dialog


CUClassDelayInfoCenterUp::CUClassDelayInfoCenterUp(CESL_Mgr* pParent /*=NULL*/)
	: CShareInfoCenterUp(pParent)
{
	//{{AFX_DATA_INIT(CUClassDelayInfoCenterUp)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT

	/// 매핑 테이블 생성하기
	m_pUClassInfoMapping = new CTableFieldMapping( this , UCLASS_DELAY_ENV  );
	
}

CUClassDelayInfoCenterUp::~CUClassDelayInfoCenterUp()
{
	delete m_pUClassInfoMapping;
}


VOID CUClassDelayInfoCenterUp::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUClassDelayInfoCenterUp)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CUClassDelayInfoCenterUp, CDialog)
	//{{AFX_MSG_MAP(CUClassDelayInfoCenterUp)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CUClassDelayInfoCenterUp message handlers



INT CUClassDelayInfoCenterUp::SetData(UPLOAD_SHARE_INFO UploadInfo , CString sLibCode ) 
{
	EFS_BEGIN
...
INT CUClassDelayInfoCenterUp::UploadData()
{
	EFS_BEGIN
...
INT CUClassDelayInfoCenterUp::InsertEnv()
{
	EFS_BEGIN
...
INT CUClassDelayInfoCenterUp::UpdateEnv()
{
	EFS_BEGIN
...
INT CUClassDelayInfoCenterUp::DeleteEnv()
{
	EFS_BEGIN
...
/// CLINET 에서 정보 구해오기
INT CUClassDelayInfoCenterUp::MakeLoaclEnvInfo()
{
	EFS_BEGIN
	...
	// 로컬 정보
	CESL_DataMgr* pDM = FindDM(_T("DM_CLIENT_UCLASS_DELAY_INFO"));
...
/// 직급별 환경 정보 INSERT
INT CUClassDelayInfoCenterUp::InsertLocalEnvToCenter()
{
	EFS_BEGIN
...
/// 직급별 환경 정보 UPDATE
INT CUClassDelayInfoCenterUp::UpdateLocalEnvToCenter()
{
	EFS_BEGIN
...
/// [4102] 직급별 환경 정보 삭제
INT CUClassDelayInfoCenterUp::DeleteCenterEnvInfo( CString sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("DELETE FROM CT_CD_CODE_TBL WHERE REC_KEY = %s ;")
		, sCenterEnvKey );
...
INT CUClassDelayInfoCenterUp::IsCenterLoanEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("SELECT REC_KEY FROM CT_CD_CODE_TBL WHERE LIB_INFO_KEY = %s AND CLIENT_REC_KEY = %s ")
...
// [4103] INSERT
INT CUClassDelayInfoCenterUp::UploadUClassDelayEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN

	INT ids;
	
	CString sTableName = _T("CT_CD_CODE_TBL");
	
	CESL_DataMgr* pLocalLoanDM = FindDM( _T("DM_CLIENT_UCLASS_DELAY_INFO") );
...
// [4203] UPDATE
INT CUClassDelayInfoCenterUp::UpdateUClassDelayEnv( CString sCenterEnvKey )
{
	EFS_BEGIN
