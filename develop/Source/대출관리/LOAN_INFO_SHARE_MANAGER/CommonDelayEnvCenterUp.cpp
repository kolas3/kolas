// CommonDelayEnvCenterUp.cpp : implementation file
//

#include "stdafx.h"
#include "CommonDelayEnvCenterUp.h"
...
/////////////////////////////////////////////////////////////////////////////
// CCommonDelayEnvCenterUp dialog


CCommonDelayEnvCenterUp::CCommonDelayEnvCenterUp(CESL_Mgr* pParent /*=NULL*/)
	: CShareInfoCenterUp( pParent)
{
	//{{AFX_DATA_INIT(CCommonDelayEnvCenterUp)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT

	/// 매핑 테이블 생성하기
	m_pUClassInfoMapping = new CTableFieldMapping( this , COMMON_DELAY_ENV  );
	
}

CCommonDelayEnvCenterUp::~CCommonDelayEnvCenterUp()
{	
	delete m_pUClassInfoMapping;
}

VOID CCommonDelayEnvCenterUp::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCommonDelayEnvCenterUp)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CCommonDelayEnvCenterUp, CDialog)
	//{{AFX_MSG_MAP(CCommonDelayEnvCenterUp)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CCommonDelayEnvCenterUp message handlers



INT CCommonDelayEnvCenterUp::SetData(UPLOAD_SHARE_INFO UploadInfo , CString sLibCode ) 
{
	EFS_BEGIN
...
INT CCommonDelayEnvCenterUp::UploadData()
{
	EFS_BEGIN
...
INT CCommonDelayEnvCenterUp::InsertEnv()
{
	EFS_BEGIN
...
INT CCommonDelayEnvCenterUp::UpdateEnv()
{
	EFS_BEGIN
...
INT CCommonDelayEnvCenterUp::DeleteEnv()
{
	EFS_BEGIN
...
/// CLINET 에서 정보 구해오기
INT CCommonDelayEnvCenterUp::MakeLoaclEnvInfo()
{
	EFS_BEGIN
	...
	// 로컬 정보
	CESL_DataMgr* pDM = FindDM(_T("DM_CLIENT_COMMON_DELAY_INFO"));
...
/// 직급별 환경 정보 INSERT
INT CCommonDelayEnvCenterUp::InsertLocalEnvToCenter()
{
	EFS_BEGIN
...
/// 직급별 환경 정보 UPDATE
INT CCommonDelayEnvCenterUp::UpdateLocalEnvToCenter()
{
	EFS_BEGIN
...
/// [4102] 직급별 환경 정보 삭제
INT CCommonDelayEnvCenterUp::DeleteCenterEnvInfo( CString sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("DELETE FROM CT_COMMON_LOAN_INFO_TBL WHERE REC_KEY = %s ;")
		, sCenterEnvKey );
	/*
...
INT CCommonDelayEnvCenterUp::IsCenterLoanEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN
	...
	CString sSql;
	sSql.Format(_T("SELECT REC_KEY FROM CT_COMMON_LOAN_INFO_TBL WHERE LIB_INFO_KEY = %s AND CLIENT_REC_KEY = %s ")
...
// [4103] INSERT
INT CCommonDelayEnvCenterUp::UploadUClassDelayEnv( CString &sCenterEnvKey )
{
	EFS_BEGIN

	INT ids;
	
	CString sTableName = _T("CT_COMMON_LOAN_INFO_TBL");
	
	CESL_DataMgr* pLocalLoanDM = FindDM( _T("DM_CLIENT_COMMON_DELAY_INFO") );
...
// [4203] UPDATE
INT CCommonDelayEnvCenterUp::UpdateUClassDelayEnv( CString sCenterEnvKey )
{
	EFS_BEGIN
